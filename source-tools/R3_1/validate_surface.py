...
OUT=Path('G:/DEM/Wenzhou_3D_Lab_R3_20260909/site/dist/r3-1/data')
contract=json.loads((OUT/'terrain.json').read_text(encoding='utf-8'))
H,W=contract['source']['shape']; ND=contract['source']['noData']
grid=np.memmap(Path('G:/DEM/Wenzhou_3D_Lab_R3_20260909/decoded-source.i16'),dtype='<i2',mode='r',shape=(H,W))
...
for p in contract['patches']:
  if not p['id'].startswith('river-'):continue
  results={}
  for skip in (1,2):
...
contract['knowledgeApplied']={'document':'XIAOMA_3D_KNOWLEDGE_R3.md','sha256':hashlib.sha256((Path('G:/DEM/Wenzhou_3D_Lab_R3_20260909/XIAOMA_3D_KNOWLEDGE_R3.md')).read_bytes()).hexdigest(),'rules':['Centre coordinates distinct from pixel edges','Original containing-cell height distinct from canonical surface value','All original samples in coarse support required','Bilinear and triangle display errors distinct','Coordinate frame distinct from physical composition and connection','Reset returns vertical exaggeration to one']}
...
(ROOT/'SURFACE_QA_R3_1.json').write_text(json.dumps(qa,ensure_ascii=False,indent=2),encoding='utf-8')

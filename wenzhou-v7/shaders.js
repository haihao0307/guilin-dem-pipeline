...
void main(){vec3 p=aPosition;float wave=.12*sin(p.x*.045+p.z*.027+uTime*.7)+.055*sin(-p.x*.071+p.z*.041+uTime*1.1);if(aField.x>.45&&aField.x<1.5)p.y=uTide+wave*.9;else if(aField.x>=1.5&&aField.x<2.5)p.y+=uTide*exp(-aField.y/22000.)+wave*.1;vWorld=p;vNormal=aNormal;vField=aField;vExtra=aExtra;gl_Position=uVP*vec4(p,1.);vDepth=1.+gl_Position.w;gl_Position.z=(log2(max(.000001,vDepth))*uLogFar-1.)*gl_Position.w;}`;

# 桂林阳朔一平方公里地貌母体精细三维样板 v0.2.0

本版直接回应第一版三维成果过粗、喀斯特读数弱、植物层干扰和材质层次不足的问题。
...
源高程交点保持原值。三维显示网格细分为 321 × 321，网格间距 3.125 米，用于承载可逆程序层。
...
田块层只在低坡、低地、湿润且避开真实水系和陡壁的位置生成田面、田埂和灌排沟渠。程序增量保持米制、可关闭和可回滚。
...
页面中没有树木、竹林、草地、植物点样和生态实例。视觉证据只来自当前 WebGL2 三维运行时。

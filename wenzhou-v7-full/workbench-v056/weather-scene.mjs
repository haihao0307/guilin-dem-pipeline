...
const WEATHER_SCENE_VERSION = '0.5.6-mobile-view-stream';
...
float densityAt(vec3 worldP){
  ...
  float h=(worldP.y-uCloudVerticalM.x)/max(1.,uCloudVerticalM.y-uCloudVerticalM.x);return smoothstep(0.025,0.72,base)*verticalEnvelope(h)*uDensityScale;
}
...
vec3 lightAt(vec3 p,vec3 rd,float d){
  ...
  vec3 ambient=mix(vec3(0.22,0.27,0.34),vec3(0.58,0.66,0.75),h);
...
void main(){
  ...
  float begin=max(hit.x,0.0),end=hit.y;float sceneZ=texture(uSceneDepth,uv).r;if(sceneZ<0.999999){float viewDistance=exp2(sceneZ*2.0/uLogFar)-1.0;end=min(end,viewDistance/max(0.0001,dot(rd,uForward)));}
  ...
  linearColor=mix(linearColor,mix(vec3(0.01,0.02,0.04),vec3(0.43,0.63,0.78),uDay),clamp(aerial,0.0,0.72));
...
function automaticWeatherFor(dateIso, hour, seed) {
  ...
  if (season === 'winter') {
    ...
    const typhoonChance = 0; // Extreme events require explicit selection or sourced event data.
...
export function createWeatherScene(gl, options = {}) {
  ...
  function draw({ vp, eye, target, viewport, mode, logFar, moving = false, sceneDepth = null, cameraAspect = null }) {
    ...
    gl.activeTexture(gl.TEXTURE0);
    gl.bindTexture(gl.TEXTURE_3D, texture);
    gl.uniform1i(uniform('uDensity'), 0);gl.activeTexture(gl.TEXTURE2);gl.bindTexture(gl.TEXTURE_2D,sceneDepth);gl.uniform1i(uniform('uSceneDepth'),2);gl.activeTexture(gl.TEXTURE0);
    ...
    gl.uniform1f(uniform('uAspect'), cameraAspect || viewport[2] / viewport[3]);
    ...
    gl.uniform1f(uniform('uStepCount'), moving?10:(state.quality==='high'?48:30));
    gl.uniform1f(uniform('uDay'), current.solar.day);gl.uniform1f(uniform('uGenus'), profile.genusCode);

...
const QC=new Map(),SC=new Map(),STC=new Map(),EBC=new Map(),PDC=new Map(),CWC=new Map();
const key=(x,z)=>`${Number(x).toFixed(4)},${Number(z).toFixed(4)}`;
function state47(x,z){const k=key(x,z);if(!STC.has(k))STC.set(k,R47.terraceStateAt(x,z));return STC.get(k)}
function predDelta(x,z){const k=key(x,z);if(!PDC.has(k))PDC.set(k,R90.terraceDelta(x,z));return PDC.get(k)}
function carrierWeight(x,z){const k=key(x,z);if(!CWC.has(k))CWC.set(k,R78.r78CarrierWeightAt(x,z));return CWC.get(k)}
...
function carrier(x,z){return carrierWeight(x,z)>1e-12}
function eligibleBench(x,z){const k=key(x,z);if(EBC.has(k))return EBC.get(k);const a=state47(x,z),ok=safe(x,z)&&a.mask>.48&&carrier(x,z)&&benchSide(a.frac)!==0;EBC.set(k,ok);return ok}
...
export function r91EnvelopeAt(x,z){const c=predDelta(x,z);let worst=0;for(const[xx,zz]of[[x+PROBE,z],[x-PROBE,z],[x,z+PROBE],[x,z-PROBE]])worst=Math.max(worst,Math.abs(c-predDelta(xx,zz)));return{worstPrior3m:worst,allowance:C((PAIR_PROOF-worst)/2,0,ADDED_CAP)}}
...
// pair endpoints independently move in the worst opposite directions. Repeated predecessor
// state/delta/carrier lookups are memoized by exact audit coordinate; this changes runtime
// cost only and does not change any geometry, gate or evidence boundary.
export function r91CorrectionAt(x,z){
 ...
 const a=state47(x,z),vals=[],axes=new Set();let far=0;
 for(const r of[3,6,9])for(const[dx,dz,axis]of[[r,0,'x'],[-r,0,'x'],[0,r,'z'],[0,-r,'z']]){
   const xx=x+dx,zz=z+dz;if(!eligibleBench(xx,zz))continue;const b=state47(xx,zz);if(!sameBenchIdentity(a,b))continue;
   vals.push(predDelta(xx,zz));axes.add(axis);if(r>=6)far++;
 }
 ...
 const centerBefore=predDelta(x,z),neighborMedian=median(vals),localResidual=neighborMedian-centerBefore,w=C(carrierWeight(x,z),0,1);
...
function compute(x,z){const p=R90.terraceStateAt(x,z),old=state47(x,z),d=r91CorrectionAt(x,z).delta;if(Math.abs(d)<=EPS)return{...p,r91Delta:0};const delta=p.delta+d,raw=old.mask>1e-9?delta/(.84*old.mask):p.raw;return{...p,raw,delta,target:p.base+delta,r91Delta:d}}

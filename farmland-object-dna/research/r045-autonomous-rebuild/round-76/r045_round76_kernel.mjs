...
function compute(x,z){
 ...
 // Both constraints must hold simultaneously. A sequential clamp can undo the first cap when intervals are disjoint.
 const lo=Math.max(env.lo,prior.delta-NODE_CAP),hi=Math.min(env.hi,prior.delta+NODE_CAP),feasible=env.feasible&&lo<=hi;
 if(!feasible)return{...prior,r76CarrierWeight:w,r76RawProfileCorrection:rawCorr,r76RawDelta:rawDd,r76Delta:0,r76EnvelopeClamped:true,r76EnvelopeFeasible:false,r76EnvelopeLo:lo,r76EnvelopeHi:hi};
 const delta=C(candidate,lo,hi),dd=delta-prior.delta,raw=old.mask>1e-9?delta/(.84*old.mask):prior.raw;
 return{...prior,raw,delta,target:prior.base+delta,r76CarrierWeight:w,r76RawProfileCorrection:rawCorr,r76RawDelta:rawDd,r76Delta:dd,r76EnvelopeClamped:Math.abs(dd-rawDd)>1e-10,r76EnvelopeFeasible:true,r76EnvelopeLo:lo,r76EnvelopeHi:hi};
}
...
export const snapshot={...R74.snapshot,version:VERSION,visualAcceptance:false,browserQA:false,productionReady:false,parcelGenerationEnabled:false,waterStateKnown:false,round76:{
 ...
 method:'reuse only the frozen R74-accepted carrier evidence and the same R75 zero-mean contour-cycle proposal. For every query point, intersect two constraints before choosing the final delta: the R74-relative +/-0.035 m node-cap interval and the four-neighbor 3 m safety intervals. The neighbor radius is 1.05 - 0.035 - 0.010 = 1.005 m; because the neighboring R76 query is itself capped to 0.035 m relative to R74, this leaves a 0.010 m proof margin under the inherited 1.05 m / 3 m cliff gate. No new footprint, family, stair, carrier, parcel, drainage route or water state is created.',
 logicCorrection:'R75 exposed an absolute-bound-versus-Lipschitz-bound error: an absolute per-node height cap of 0.035 m does not imply a relative 3 m neighbor-difference cap. The first R76 attempt then exposed a second constraint-composition error: clamping a candidate to the node cap and afterwards clamping it to a neighbor interval can undo the node cap if the intervals do not coincide. R76 now intersects both admissible intervals first and only then clamps once. It neither lowers the 1.05 m QA threshold nor assumes that passing this local relation proves whole-slope visual acceptance.',

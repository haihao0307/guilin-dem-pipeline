...
function acceptedBit(x,z){if(x<X0||x>X1||z<Z0||z>Z1)return 0;return latticeInfoExact(x,z).accepted?1:0}
function acceptanceWeightAt(x,z){
 ...
 const a=acceptedBit(xa,za),b=acceptedBit(xa+STEP,za),c=acceptedBit(xa,za+STEP),d=acceptedBit(xa+STEP,za+STEP);
 return M(M(a,b,tx),M(c,d,tx),tz);
}
function compute(x,z){
 const prior=R66.terraceStateAt(x,z),proposal=R67.terraceStateAt(x,z),cand=candidateAt(x,z),weight=acceptanceWeightAt(x,z),comp=latticeInfoExact(Math.round((x-X0)/STEP)*STEP+X0,Math.round((z-Z0)/STEP)*STEP+Z0);
 if(weight<=1e-12)return{...prior,r68Candidate:cand.ok,r68Accepted:false,r68AcceptanceWeight:0,r68ComponentSize:comp.size,r68ComponentSpan:comp.span,r68ProposalDelta:proposal.delta-prior.delta};
 const raw=M(prior.raw,proposal.raw,weight),delta=M(prior.delta,proposal.delta,weight);
 return{...prior,raw,delta,target:prior.base+delta,r68Candidate:cand.ok,r68Accepted:weight>.999999,r68AcceptanceWeight:weight,r68ComponentSize:comp.size,r68ComponentSpan:comp.span,r68ProposalDelta:proposal.delta-prior.delta,r68PriorDelta:prior.delta};
}

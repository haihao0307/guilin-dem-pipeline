...
for(const x of xs)for(const z of zs){
 ...
 maxChange=Math.max(maxChange,d);if(d>1e-6){changed++;changedPts.push({x,z,change:n.delta-p.delta,mask:b.mask,groupIndex:b.groupIndex,coverageGain:n.coverageProfileGain||0});if(b.mask>.22&&b.mask<.64){mediumChanged++;changedGroups.add(b.groupIndex);coverageGainSum+=n.coverageProfileGain||0;coverageGainMax=Math.max(coverageGainMax,n.coverageProfileGain||0)}else unsupportedChanged++;if(dd<=12)hardChanged++;const gap=Math.abs(z-K.riverZ(x)),w=K.riverW(x);if(gap<=w+12)receiverChanged++;if(dd<12)unsafeChanged++;}
}
for(const p of changedPts){const{x,z}=p;maxInc=Math.max(maxInc,Math.abs(K.terraceDelta(x+3,z)-K.terraceDelta(x,z)),Math.abs(K.terraceDelta(x-3,z)-K.terraceDelta(x,z)),Math.abs(K.terraceDelta(x,z+3)-K.terraceDelta(x,z)),Math.abs(K.terraceDelta(x,z-3)-K.terraceDelta(x,z)))}

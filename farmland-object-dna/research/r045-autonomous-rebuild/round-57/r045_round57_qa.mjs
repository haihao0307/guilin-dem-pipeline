...
let active=0,groups=[0,0,0],maskChange=0,stepChange=0,phaseChange=0,indexChange=0,baseChange=0,strongErr=0,maxChange=0,changed=0,mediumChanged=0,coverageGainSum=0,coverageGainMax=0,unsafeChanged=0,unsupportedChanged=0,hardChanged=0,receiverChanged=0,maxInc=0;const changedGroups=new Set();
for(const x of xs)for(const z of zs){
 ...
 maxChange=Math.max(maxChange,d);if(d>1e-6){changed++;if(b.mask>.22&&b.mask<.64){mediumChanged++;changedGroups.add(b.groupIndex);coverageGainSum+=n.coverageProfileGain||0;coverageGainMax=Math.max(coverageGainMax,n.coverageProfileGain||0)}else unsupportedChanged++;if(dd<=12)hardChanged++;const gap=Math.abs(z-K.riverZ(x)),w=K.riverW(x);if(gap<=w+12)receiverChanged++;if(dd<12)unsafeChanged++;}
 maxInc=Math.max(maxInc,Math.abs(K.terraceDelta(x+3,z)-K.terraceDelta(x,z)),Math.abs(K.terraceDelta(x,z+3)-K.terraceDelta(x,z)));
}
...
add('medium_support_material',mediumChanged>=8,{mediumChanged,changed,active},'>=8 changed 6m audit cells in inherited medium support');
...
add('three_meter_cliff_bound',maxInc<1.05,maxInc,'<1.05m terrace increment / 3m');
add('three_groups_retained',groups.every(v=>v>20),groups,'three groups material');
...
const result={version:K.VERSION,passed:checks.every(c=>c.pass),gateCount:checks.length,passedCount:checks.filter(c=>c.pass).length,checks,metrics:{active,groups,mediumChanged,changed,changedGroups:[...changedGroups],avgCoverageGain,coverageGainMax,strongErr,maxChange,unsupportedChanged,hardChanged,receiverChanged,maxInc,maskChange,stepChange,phaseChange,indexChange,baseChange,qaScope:'6m whole-slope plan-frame audit; medium-support profile coverage only; browser fixed view built separately'}};fs.writeFileSync(new URL('./r045_round57_qa_result.json',import.meta.url),JSON.stringify(result,null,2));console.log(JSON.stringify(result,null,2));if(!result.passed)process.exitCode=2;

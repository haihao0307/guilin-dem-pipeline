...
import * as B from '../round-38/r045_round38_kernel.mjs';
...
const px0=-220,px1=120,pz0=-132,pz1=10,pnx=34,pnz=12,plan=[];
...
let dmax=0,active=0,gains=0,gc=[0,0,0];for(let x=-210;x<=110;x+=12)for(let z=-128;z<=2;z+=8){const st=K.terraceStateAt(x,z),old=B.terraceStateAt(x,z);dmax=Math.max(dmax,Math.abs(st.delta));if(st.mask>.12){active++;if(st.groupIndex>=0&&st.groupIndex<3)gc[st.groupIndex]++}if(st.mask-old.mask>.004)gains++}
const data={version:K.VERSION,terrain:{x0,x1,z0,z1,nx,nz,before,after},rivers,planSpec:{x0:px0,x1:px1,z0:pz0,z1:pz1,nx:pnx,nz:pnz},plan,state:{dmax,active,gains,groups:gc},generatedAt:new Date().toISOString(),source:'R045.39 kernel/R045.38 baseline'};
fs.writeFileSync(new URL('./r045_round39_audit_cache.mjs',import.meta.url),`export const AUDIT=${JSON.stringify(data)};\n`);console.log(JSON.stringify({version:data.version,terrainVertices:(nx+1)*(nz+1),planCells:plan.length,state:data.state},null,2));

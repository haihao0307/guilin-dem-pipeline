...
const EXPECT=K.R86_CONTRACT,src=process.env.GITHUB_SHA||'local',PROOF=1.045,CLIFF=1.05,CAP=.012,EPS=1e-6;
...
// Authoritative morphology objective: real +/-3 m pairs that remain on one broad
// bench side, same family and exact stair. Count each undirected pair once.
let benchPairs=[],beforeGrad=[],afterGrad=[],improvedPairs=0,worsenedPairs=0;const benchSeen=new Set(),opportunityGroups=new Set();
for(const c of cand){const a=R47.terraceStateAt(c.x,c.z);for(const[xx,zz]of[[c.x+3,c.z],[c.x-3,c.z],[c.x,c.z+3],[c.x,c.z-3]]){const b=R47.terraceStateAt(xx,zz),sa=side(a.frac),sb=side(b.frac);if(!sa||sa!==sb)continue;if(a.groupIndex!==b.groupIndex||a.index!==b.index)continue;if(R30.nearestExtendedDrainageDistance(xx,zz)<=12||receiver(xx,zz))continue;const p0=sk(c.x,c.z),p1=sk(xx,zz),pk=p0<p1?`${p0}|${p1}`:`${p1}|${p0}`;if(benchSeen.has(pk))continue;benchSeen.add(pk);const gb=Math.abs(R84.terraceDelta(c.x,c.z)-R84.terraceDelta(xx,zz)),ga=Math.abs(K.terraceDelta(c.x,c.z)-K.terraceDelta(xx,zz));benchPairs.push({x:c.x,z:c.z,xx,zz,before:gb,after:ga,side:sa,groupIndex:a.groupIndex,index:a.index});opportunityGroups.add(a.groupIndex);beforeGrad.push(gb);afterGrad.push(ga);if(ga<gb-1e-9)improvedPairs++;else if(ga>gb+1e-9)worsenedPairs++}}
...
// Safety domain correction: R86 attempt 1 exposed an inherited R84 3 m pair at
// 1.0864 m whose two endpoints received zero R86 correction. Requiring every pair
// in this newly expanded audit domain to be <=1.045 m falsely assigns predecessor
// debt to the child. We therefore keep the fixed 1.045/1.05 gates for every pair
// that R86 actually touches, and separately require zero worsening of any inherited
// over-limit pair. Thresholds are not relaxed.
let allPairs=[],pairSeen=new Set(),maxBeforeInc=0,maxAfterInc=0,maxGlobalWorsening=-Infinity,maxGlobalWorseningAt=null,maxChangedAfter=0,maxChangedBefore=0,maxChangedAt=null,changedPairCount=0,newProofViolation=0,newCliffViolation=0,inheritedUnsafeTouched=0,inheritedUnsafeWorsened=0;
for(const p of samples.values())for(const[xx,zz]of[[p.x+3,p.z],[p.x-3,p.z],[p.x,p.z+3],[p.x,p.z-3]]){
 ...
 const b=Math.abs(R84.terraceDelta(p.x,p.z)-R84.terraceDelta(xx,zz)),a=Math.abs(K.terraceDelta(p.x,p.z)-K.terraceDelta(xx,zz)),d0=Math.abs(K.r86CorrectionAt(p.x,p.z).delta),d1=Math.abs(K.r86CorrectionAt(xx,zz).delta),touched=d0>1e-7||d1>1e-7,w=a-b;
 const rec={x:p.x,z:p.z,xx,zz,before:b,after:a,worsening:w,touched,d0,d1};allPairs.push(rec);maxBeforeInc=Math.max(maxBeforeInc,b);maxAfterInc=Math.max(maxAfterInc,a);if(w>maxGlobalWorsening){maxGlobalWorsening=w;maxGlobalWorseningAt=rec}
 if(touched){changedPairCount++;if(a>maxChangedAfter){maxChangedAfter=a;maxChangedBefore=b;maxChangedAt=rec}if(b<=PROOF+EPS&&a>PROOF+EPS)newProofViolation++;if(b<CLIFF-EPS&&a>=CLIFF)newCliffViolation++;if(b>PROOF+EPS){inheritedUnsafeTouched++;if(a>b+EPS)inheritedUnsafeWorsened++}}
}
...
ck('distributed_materiality',changed.length>=40&&nodeChanged>=24&&sameSet(groups,opportunityGroups)&&sides.has(-1)&&sides.has(1),{sampleCount:samples.size,changed:changed.length,nodeChanged,changedGroups:[...groups],opportunityGroups:[...opportunityGroups],allInheritedFamilies:[...new Set(cand.map(c=>c.groupIndex))],sides:[...sides]},'>=40 changed queries, >=24 nodes, every family with a valid same-identity broad-bench pair, both bench sides');
ck('broad_bench_pair_domain',benchPairs.length>=24,{pairs:benchPairs.length,groups:[...opportunityGroups],beforeMean:benchBefore,afterMean:benchAfter},'>=24 same-identity broad-bench 3m pairs');
...
ck('changed_pair_proof',changedPairCount>0&&newProofViolation===0&&inheritedUnsafeWorsened===0,{changedPairCount,maxChangedBefore,maxChangedAfter,maxChangedAt,newProofViolation,inheritedUnsafeTouched,inheritedUnsafeWorsened},'R86-touched pair: <=1.045m when predecessor was <=1.045m; inherited over-limit pair may not worsen');
ck('cliff_no_new_violation',newCliffViolation===0&&inheritedUnsafeWorsened===0,{newCliffViolation,inheritedUnsafeWorsened,maxChangedAfter,maxChangedAt},'no R86-touched pair may newly cross 1.05m; inherited over-limit pair may not worsen');
ck('global_pair_no_worsening',maxAfterInc<=maxBeforeInc+EPS&&maxGlobalWorsening<=2*CAP+EPS,{maxBeforeInc,maxAfterInc,maxGlobalWorsening,maxGlobalWorseningAt},'global inherited maximum must not increase; local pair worsening bounded by correction caps');
...
console.log(JSON.stringify({version:K.VERSION,passed,passedCount:result.passedCount,gateCount:result.gateCount,samples:samples.size,changedQueries:changed.length,nodeChanged,changedGroups:[...groups],opportunityGroups:[...opportunityGroups],sides:[...sides],benchPairs:benchPairs.length,benchBefore,benchAfter,benchRatio,energyRatio,improvedPairs,worsenedPairs,balanceRatio:metrics.balanceRatio,maxChange,projected,changedPairCount,maxChangedAfter,newProofViolation,newCliffViolation,inheritedUnsafeTouched,inheritedUnsafeWorsened,maxBeforeInc,maxAfterInc,maxGlobalWorsening,guardLeak},null,2));

...
export const snapshot={...R66.snapshot,version:VERSION,visualAcceptance:false,browserQA:false,productionReady:false,parcelGenerationEnabled:false,waterStateKnown:false,round67:{
 ...
 method:'use only frozen R47 evidence on the authoritative lattice. At each existing active cell, select the best of eight lattice directions for each sign of the local R30 contour tangent, then sample one/two/three frozen lattice steps. Every support must remain same-family, step/index compatible and clear the inherited drainage/receiver protection. Real physical reach uses hypot(), so diagonal steps are never mislabeled as 6 m. Convert bilateral reach/support/alignment into a bounded coherence weight, add at most 0.20 profile-blend coverage and cap R67-R66 local delta change at 0.045 m. R67 output never seeds its own evidence.',
 logicCorrection:'R66 proving physically real local corrections does not imply the hillside reads as a coherent terrace system. Counting changed cells is a materiality check, not a macro-organization proof. The first R67 probe also exposed a computational mistake: sampling arbitrary floating contour coordinates destroys frozen-lattice cache reuse and tests a different support domain from the authoritative 6 m audit. R67 now tests connected physical span on the same frozen lattice rather than buying evidence with off-grid evaluations.',
 constraint:'the one/two/three lattice-step support probes, 0.70 tangent-alignment gate, 0.18 coherence floor, 0.20 blend cap, 0.045 m per-point change cap, 6 m audit lattice and inherited 12 m drainage core are synthetic morphology/QA controls, not surveyed Yunnan terrace dimensions. A 12.5 m macro DEM plus photographs cannot recover metre/sub-metre field microtopography, real parcel/management boundaries, measured bund/riser/channel sections, inlet/outlet invert elevations, observed hydraulic connectivity, head/depth/discharge/gate states or event-level water management.',

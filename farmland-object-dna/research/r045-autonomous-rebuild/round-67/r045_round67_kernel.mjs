...
function compute(x,z){
 ...
 if(old.mask<=.12||old.mask>=.82||!safeFrozen(x,z))return{...prior,runCoherenceGain:0,runCoherence:0,runSpan:0,runProfileCorrection:0};
 const rb=runBlendAt(x,z,old);if(rb.gain<=1e-12)return{...prior,runCoherenceGain:0,runCoherence:rb.coherence,runSpan:rb.span,runProfileCorrection:0};
 const f=old.frac,corr=runProfileCorrection(f,old.step)*rb.gain;
 if(Math.abs(corr)<=1e-12)return{...prior,runCoherenceGain:rb.gain,runCoherence:rb.coherence,runSpan:rb.span,runSupports:rb.supports,runProfileCorrection:0};
 ...
 return{...prior,raw,delta,target:old.base+delta,runCoherenceGain:rb.gain,runCoherence:rb.coherence,runSpan:rb.span,runSupports:rb.supports,runLeft:rb.left,runRight:rb.right,runProfileCorrection:corr,runPriorDelta:prior.delta};
}

...
export function agriculturalFaceDelta(x,z){
  ...
  // The first R21 candidate proved too faint: its mean lower-slope response was only ~0.052 m and
  // left/right mean separation only ~0.023 m even though the centroid was displaced. That failure
  // means the intended *macro* face was not broad/legible enough. Do not weaken QA; scale the whole
  // causal field coherently instead. The 1.65 gain preserves support, drainage protection and phase
  // relationships while bringing the broad one-sided mass above the predeclared gates.
  return C(1.65*env*drainClear*divideEase*(bay+shoulder+counter+toe),-1.05,1.18);
}
...
export const snapshot={
  ...
  round21:{
    ...
    firstCandidateFailure:'initial R21 field was numerically too faint to qualify as broad one-sided macro massing; QA stayed fixed and the geometry amplitude was increased coherently',

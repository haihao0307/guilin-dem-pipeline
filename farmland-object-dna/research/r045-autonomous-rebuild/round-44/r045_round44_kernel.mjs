...
function compatible(a,b){
 ...
 return Math.abs(a.step-b.step)<=Math.max(.22,.30*step)&&Math.abs(a.index-b.index)<=4;
}
...
function pathSafe(x,z,ux,uz,n,sgn){
 for(let i=1;i<n;i++){
  const px=x+sgn*ux*i,pz=z+sgn*uz*i;
  if(safetyAt(px,pz)<=.08)return false;
 }
 return true;
}
function findAnchor(base,x,z,ux,uz,sgn,minN=2,maxN=6){
 for(let n=minN;n<=maxN;n++){
  const q=r43At(x+sgn*ux*n,z+sgn*uz*n);
  if(q.mask>.12&&compatible(base,q)&&pathSafe(x,z,ux,uz,n,sgn))return{q,n,x:x+sgn*ux*n,z:z+sgn*uz*n};
 }
 return null;
}
function bridgeSupportAt(x,z){
 const base=r43At(x,z);if(base.mask>.12||safetyAt(x,z)<=0)return null;
 let best=null;
 for(const [ux,uz,name] of [[6,0,'contour-x'],[6,6,'diag-down'],[6,-6,'diag-up']]){
  const a=findAnchor(base,x,z,ux,uz,-1,2,6),b=findAnchor(base,x,z,ux,uz,1,2,6);
  if(!a||!b||!compatible(a.q,b.q))continue;
  const span=(a.n+b.n)*Math.hypot(ux,uz);
  const strength=Math.min(a.q.mask,b.q.mask)+.35*safetyAt(x,z)-.0025*span;
  if(!best||strength>best.strength)best={mode:'two-anchor-corridor',name,ux,uz,leftN:a.n,rightN:b.n,span,leftMask:a.q.mask,rightMask:b.q.mask,strength,groupIndex:base.groupIndex};
 }
 return best;
}
...
export function macroCorridorGain(x,z){
 const base=r43At(x,z);if(base.mask>.12)return 0;
 const sup=macroCorridorSupportAt(x,z);if(!sup)return 0;
 const safe=safetyAt(x,z);
 const target=sup.mode==='two-anchor-corridor'?C(.28+.055*safe+.055*Math.min(sup.leftMask,sup.rightMask),.29,.42):C(.205+.05*safe+.035*Math.min(...sup.sourceMasks),.22,.31);
 const rawAbs=Math.abs(base.raw),deltaBound=rawAbs>1e-9?.28/(.84*rawAbs):.34;
 return C(Math.max(0,target-base.mask),0,Math.min(.34,deltaBound));
}
function computeTerraceState(x,z){
 const base=r43At(x,z);
 if(base.mask>.12)return{...base,macroCorridorGain:0,macroCorridorSupport:null};
 const gain=macroCorridorGain(x,z),mask=C(base.mask+gain,0,1),delta=.84*mask*base.raw,sup=gain>0?macroCorridorSupportAt(x,z):null;
 return{...base,mask,delta,target:base.base+delta,macroCorridorGain:gain,macroCorridorSupport:sup};
}
...
export const snapshot={...R43.snapshot,version:VERSION,visualAcceptance:false,browserQA:false,productionReady:false,parcelGenerationEnabled:false,terraceGeometryEnabled:true,terracePilotPreviewEnabled:true,waterStateKnown:false,round44:{
 scope:'raise terrace organization from sparse one-cell branch repairs to frozen-R43 macro contour corridors across the one-sided agricultural slope; retain vertical stair response and all hydrology protections',
 method:'existing R43 active terrace samples remain exact. Weak frozen-R43 samples may be promoted only by either same-family/stair-compatible anchors on both sides 12-36 m away along one of three contour-oriented lattice directions with every intervening sample safe, or by a frozen three-active-cell stable chain that extends one cell. R44 cells never seed more R44 cells. The 0.84 multiplier, stair step, phase and raw response are inherited unchanged.',
 logicCorrection:'main-view terrace weakness does not imply risers should be amplified: visibility and topological organization are different variables. Likewise, longer ribbons alone do not prove agricultural correctness because an indiscriminate bridge can erase real drainage separators. R44 therefore changes only frozen support organization, keeps vertical quantization fixed, and requires every promoted corridor to re-pass drainage and foreground-receiver safety.',
 constraint:'the 6 m QA lattice, 12-36 m synthetic anchor search, three lattice directions, 12 m hard drainage core and generated corridor continuations are morphology/QA parameters, not surveyed Yunnan terrace dimensions or measured hydraulic connectivity. Current 12.5 m macro DEM and photographs cannot provide field microtopography, parcel/management boundaries, bund-riser-channel sections, inlet/outlet sill elevations, hydraulic connectivity or event water-management records.',
 ...
 evidenceClass:'synthetic frozen-R43 macro contour-corridor organization; not surveyed terrace, parcel or hydraulic truth',

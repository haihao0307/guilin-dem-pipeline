...
export const receivingHierarchy=R25.apronProfiles.map((p,i)=>({
  ...
  bend:[34,22,-27][i],
  depth:[.128,.098,.073][i],
  shoulder:[.058,.046,.038][i],
  nested:[.052,.039,.031][i],
  edgeSign:[1,-1,1][i],
...
export const snapshot={
  ...
  round26:{
    ...
    qaCorrection:'the first R26 runner showed that declared shoulders were fully cancelled because they sat inside the broad receiving hollow, so the geometry moved them outside without weakening that gate. The second runner then showed the major and subordinate occupied footprints were still too similar (401 vs 379 samples); the subordinate lobe was narrowed in geometry rather than relaxing the >12% hierarchy gate.',

...
import * as K from './r045_round61_kernel.mjs';
...
let qr={metrics:{changedPoints:[]}};try{qr=JSON.parse(fs.readFileSync(new URL('./r045_round61_qa_result.json',import.meta.url),'utf8'))}catch{}
const exactChanges=(qr.metrics?.changedPoints||[]).filter(p=>Math.abs(p.change)>1e-6);
const plan=[];for(let z=z0;z<=z1;z+=6)for(let x=x0;x<=x1;x+=6){const n=K.terraceStateAt(x,z),p=B.terraceStateAt(x,z);plan.push({x,z,mask:n.mask,priorMask:p.mask,groupIndex:n.groupIndex,frac:n.frac,dd:K.nearestExtendedDrainageDistance(x,z),gain:n.contourBandGain||0,mode:n.contourBandSupport?.mode||null,seedX:n.contourBandSupport?.seedX??null,seedZ:n.contourBandSupport?.seedZ??null,seedDistance:n.contourBandSupport?.seedPhysicalDistance??null})}
const data={version:K.VERSION,terrain:{x0,x1,z0,z1,nx,nz,before,after},rivers,plan,exactChanges,metrics:qr.metrics||{},generatedAt:new Date().toISOString(),source:'R045.61 fixed view: A=accepted R58, B=bounded frozen-R58 contour-support band; orange markers are authoritative 6m promotions; blue-grey preserves hard drainage core'};
fs.writeFileSync(new URL('./r045_round61_audit_cache.mjs',import.meta.url),`export const AUDIT=${JSON.stringify(data)};\n`);console.log(JSON.stringify({version:data.version,terrainVertices:(nx+1)*(nz+1),planCells:plan.length,exactChanges:exactChanges.length},null,2));

...
// and recomputing multiple permission fields still made QA unnecessarily slow. Final R34 never creates support
// where R33 has zero support. It only raises weak positive support with a bounded monotone remap, slightly more
// at inherited family-overlap junctions. Step, phase, raw stair response and 0.84 amplitude remain exactly R33.
...
export const snapshot={
  ...
  round34:{
    scope:'reduce the remaining R33 terrace-cluster reading by strengthening weak positive support inside the already-existing terrace families, with a modest extra lift at inherited family overlaps; hard drainage interruptions, parcels and hydraulics remain locked',
    method:'hold R33 footprint zeros, step, phase, raw stair response and 0.84 vertical amplitude exactly fixed; apply one bounded monotone remap to positive R33 mask values, using only inherited adjacent-family overlap to vary the exponent from 1.20 to 1.40. Because zero maps to zero, the operation cannot invent new terrace footprint across drainage or river exclusions.',
    logicCorrection:'R33 remaining cluster visibility does not prove that risers are too low or that terrace amplitude should increase. It also does not justify unrestricted dilation into new terrain. Two earlier R34 formulations further confused geometric sophistication with useful computation by repeatedly recomputing off-axis or duplicated permission fields. Final R34 tests the smallest causal change: strengthen only already-positive inherited support and leave all zero-support boundaries and elevation quantization unchanged.',
    failedAttempt:'the first R34 implementation used local R30 gradient tangents plus six off-axis permission probes per terrace query; a second formulation still recomputed duplicated permission fields. Both kept numeric generation running for minutes rather than the normal round budget. They remain in Git history but were rejected instead of treating compute cost as irrelevant.',

...
import { initializeIntake } from './intake.js';
...
function createRegionCard(region) {
  ...
  card.querySelector('[data-mesh]').textContent = `卡片 ${region.render.cardMesh}²，单独查看最高 ${region.render.focusMesh}²`;
  ...
  const metrics = { set textContent(value) { const parts = value.split(' · '); distance.textContent = parts[0] || ''; grid.textContent = parts.slice(1).join(' · '); } };
  const viewer = new TerrainRenderer(card.querySelector('[data-canvas]'), region, { readout: card.querySelector('[data-readout]'), metrics, fallback: card.querySelector('[data-fallback]') });
...
async function openFocus(region) {
  ...
  canvas.hidden = false; fallback.hidden = true; dialog.showModal();
  const viewer = new TerrainRenderer(canvas, region, { focus: true, readout: document.getElementById('focusReadout'), metrics: document.getElementById('focusMetrics'), fallback });
...
function closeFocus() {
  if (state.focusViewer) { state.focusViewer.dispose(); state.focusViewer = null; }
  state.activeRegion = null;
  document.getElementById('focusDialog').close();
}

document.getElementById('closeFocus').addEventListener('click', closeFocus);
document.getElementById('focusDialog').addEventListener('cancel', (event) => { event.preventDefault(); closeFocus(); });
...
const creations = manifest.regions.map((region) => { const creation = createRegionCard(region); regionGrid.append(creation.card); return { region, ...creation }; });
const results = await Promise.allSettled(creations.map(async ({ region, viewer }) => {
  try { await viewer.load(); return region.id; }
  catch (error) { state.failures.push({ region: region.id, message: error.message }); state.cards.get(region.id).querySelector('[data-readout]').textContent = `载入失败：${error.message}`; throw error; }
}));
...
if (failures.length) {
  ...
  document.getElementById('globalStatus').textContent = '三地区高精度真实地貌已经载入';
  document.getElementById('globalDetail').textContent = '可直接旋转、连续缩放、切换诊断层并单独精细查看';

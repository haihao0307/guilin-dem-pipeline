...
function showFatal(error) {
  ...
  window.__WENZHOU_V111_DIAGNOSTICS__ = {
    ready: false,
    fatalError: message,
...
function computeNormals(positions, grid, spacing) {
  ...
  for (let row = 0; row < grid; row += 1) {
    for (let col = 0; col < grid; col += 1) {
      ...
      const normal = Vec3.normalize([left - right, 2 * spacing, north - south]);
...
const state = {
  ...
  riverWidth: 1, waterOpacity: .76, currentAnchor: 'overall', flight: null, keys: new Set(), pointer: null,
...
function sampleMeshTerrain(mesh, x, z) {
  if (!mesh) return null;
...
function flyTo(id){const anchor=anchors[id];if(!anchor)return;void loadDetailTile(id);state.camera.ground=false;state.currentAnchor=id;const terrainY=sampleMarine(anchor.target[0],anchor.target[2])?1:sampleTerrain(anchor.target[0],anchor.target[2]),to={...anchor,target:[anchor.target[0],Math.max(anchor.target[1],terrainY),anchor.target[2]]};state.flight={started:performance.now(),duration:1450,from:{target:[...state.camera.target],distance:state.camera.distance,azimuth:state.camera.azimuth,elevation:state.camera.elevation},to};document.querySelectorAll('[data-anchor]').forEach(button=>button.classList.toggle('active',button.dataset.anchor===id));ui.cameraModeLabel.textContent=`${anchor.label} · 连续飞行`;}

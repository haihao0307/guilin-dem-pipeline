...
const support=r.audit.crownSoilSupport;
assert.equal(support.method,'closed vertical extrusion of final upward rock triangles');assert.ok(support.selectedRockTriangles>1000&&support.lowerVertexCount>1000);assert.equal(support.maximumUnsupportedGapM,0);assert.ok(support.maximumOffsetErrorM<2e-6);
...
const report={...r.audit,generatedBufferSha256:digest,bounds,dimensions,deterministicRepeat:true,sourcePhotoObservationOnly:true,originalGlbMeasuredThisRun:false,actualBrowserExecuted:false,testsPassed:12,testsFailed:0};fs.writeFileSync(out+'/numeric.json',JSON.stringify(report,null,2));fs.writeFileSync(out+'/habitat-interface.json',JSON.stringify({schema:'landscape-mother/habitat/1',units:'authored-metres',sites:r.anchors},null,2));console.log(JSON.stringify(report,null,2));

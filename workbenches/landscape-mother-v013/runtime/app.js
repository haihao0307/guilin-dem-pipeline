...
try{load().catch(reason=>{console.error(reason);fail('数值地貌读取失败：'+reason.message)});setView('hero');setTour(true);requestAnimationFrame(loop)}catch(reason){console.error(reason);fail('工作台启动失败：'+reason.message)}
})();

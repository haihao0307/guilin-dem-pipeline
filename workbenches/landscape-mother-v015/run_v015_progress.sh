...
TRANSFORM="$ROOT/workbenches/landscape-mother-v015/transform_from_v014.py"
BROWSER_QA="$ROOT/workbenches/landscape-mother-v015/browser_qa_v015.py"
BASE_A="$RUNNER_TEMP/landscape-mother-v015-base-a"
BASE_B="$RUNNER_TEMP/landscape-mother-v015-base-b"
OUT_A="$RUNNER_TEMP/landscape-mother-v015-progress"
OUT_B="$RUNNER_TEMP/landscape-mother-v015-progress-repeat"
SITE="$RUNNER_TEMP/landscape-mother-v015-site"
SOURCE_URL="https://haihao0307.github.io/guilin-dem-pipeline/landscape-mother-v014"
PUBLIC_DIR="landscape-mother-v015-progress"
PUBLIC_URL="https://haihao0307.github.io/guilin-dem-pipeline/$PUBLIC_DIR"

export OUT_A OUT_B PUBLIC_URL

printf '\n[V015] fetch verified V014 runtime\n'
mkdir -p "$BASE_A" "$BASE_B" "$OUT_A" "$OUT_B"
FILES=(index.html styles.css app.js scene.bin SCENE_META.json SOURCE_RECEIPT.json README.md SCALE_VERTICALITY_KNOWLEDGE_R015.md ONLINE_RELEASE.json)
for file in "${FILES[@]}"; do
  curl -fsSL --retry 6 --retry-all-errors --connect-timeout 20 \
    "$SOURCE_URL/$file?source=v015-${GITHUB_RUN_ID}" -o "$BASE_A/$file"
done
python - <<'PY'
import hashlib,json,os
from pathlib import Path
root=Path(os.environ['RUNNER_TEMP'])/'landscape-mother-v015-base-a'
...
print('verified V014 source release',release['release'])
PY
cp -a "$BASE_A/." "$BASE_B/"
cp -a "$BASE_A/." "$OUT_A/"
cp -a "$BASE_B/." "$OUT_B/"

printf '\n[V015] deterministic tower reconstruction x2\n'
python "$TRANSFORM" "$OUT_A"
python "$TRANSFORM" "$OUT_B"
...
python -m py_compile "$TRANSFORM" "$BROWSER_QA"
python - <<'PY'
import json,os
from pathlib import Path
...
printf '\n[V015] local mobile and desktop WebGL QA\n'
python -m http.server 4175 --directory "$OUT_A" >"$RUNNER_TEMP/v015-http.log" 2>&1 &
SERVER_PID=$!
cleanup_server(){ kill "$SERVER_PID" 2>/dev/null || true; }
trap cleanup_server EXIT
LANDSCAPE_OUT="$OUT_A" LANDSCAPE_URL="http://127.0.0.1:4175/?scene=fenglin&v=v015-progress" python "$BROWSER_QA"
cleanup_server
trap - EXIT
cp "$OUT_A/BROWSER_QA.json" "$OUT_A/LOCAL_BROWSER_QA.json"

printf '\n[V015] prepare progress release receipt\n'
python - <<'PY'
import hashlib,json,os
from pathlib import Path
root=Path(os.environ['OUT_A'])
...
receipt={
 'schema':'landscape-mother-v015-progress-release/1',
 'release':'guilin-putao-fenglin-v015-progress-20260904-p1',
 'sourceCommit':os.environ['GITHUB_SHA'],
 'sourceRuntime':'guilin-putao-fenglin-v014-20260903-r3',
 'publicUrl':os.environ['PUBLIC_URL']+'/?scene=fenglin&v=v015-progress-p1',
...
printf '\n[V015] scoped gh-pages publication\n'
...
git add "$PUBLIC_DIR"
git commit -m "feat(landscape-mother): publish Guilin Putao V015 progress candidate"
...
printf '\n[V015] verify public HTTPS bytes\n'
...
for attempt in $(seq 1 48); do
  ...
  if curl -fsSL "$PUBLIC_URL/scene.bin?cb=$cache" -o "$RUNNER_TEMP/public-v015-scene.bin" \
    && curl -fsSL "$PUBLIC_URL/app.js?cb=$cache" -o "$RUNNER_TEMP/public-v015-app.js" \
    && [ "$(sha256sum "$RUNNER_TEMP/public-v015-scene.bin" | awk '{print $1}')" = "$EXPECTED_SCENE" ] \
    && [ "$(sha256sum "$RUNNER_TEMP/public-v015-app.js" | awk '{print $1}')" = "$EXPECTED_JS" ]; then
      ok=1
      break
  fi
  sleep 10
...
printf '\n[V015] public mobile and desktop WebGL QA\n'
...
printf '\n[V015] publish final public receipt\n'
cd "$SITE"
git fetch origin gh-pages
git rebase origin/gh-pages
...
git commit -m "test(landscape-mother): record V015 public browser proof"
if ! git push origin HEAD:gh-pages; then
  git fetch origin gh-pages
  git rebase origin/gh-pages
  git push origin HEAD:gh-pages
...
meta=json.loads((root/'SCENE_META.json').read_text())
release=json.loads((root/'ONLINE_RELEASE.json').read_text())
print('## Landscape Mother Guilin Putao V015 progress')

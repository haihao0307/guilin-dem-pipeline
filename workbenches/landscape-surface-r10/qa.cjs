...
await page.goto('http://127.0.0.1:8798/workbenches/landscape-surface-r10/index.html#micro');
...
await frame('micro default');await shot('r10-micro');const defaultCrop=await crop();
await page.evaluate(()=>__LM__.setMaterial({scope:0,micro:0}));await frame('both detail controls off');await shot('r10-all-off');const offCrop=await crop();
await page.evaluate(()=>__LM__.setMaterial({scope:1,micro:0}));await frame('microscope independent of legacy micro switch');await shot('r10-microscope-only');const scopeCrop=await crop();assert(!scopeCrop.equals(offCrop));checks.push('visible microscope-only image difference inside canvas');
...
await page.mouse.move(550,420);await page.mouse.wheel(0,-1450);await page.waitForTimeout(250);assert((await page.evaluate(()=>__LM__.state.radius))<.5);await frame('sub-metre zoom');await shot('r10-ultra');
await page.mouse.wheel(0,-10000);await page.waitForTimeout(250);assert.equal(await page.evaluate(()=>__LM__.state.radius),.12);await frame('closest allowed zoom');await shot('r10-minimum-zoom');
await page.evaluate(s=>__LM__.restoreState(s),focus.state);const before=await page.evaluate(()=>__LM__.state.theta);
await page.mouse.move(550,420);await page.mouse.down();await page.mouse.move(640,440,{steps:8});await page.mouse.up();await page.waitForTimeout(250);assert.notEqual(await page.evaluate(()=>__LM__.state.theta),before);await frame('near-surface mouse orbit');await shot('r10-micro-side');
...
for(const view of ['hero','cliff','summit','cave','foot','back','stone','section']){await page.locator('[data-view="'+view+'"]').click();await frame(view);await shot('r10-'+view);}
await page.setViewportSize({width:390,height:844});await page.locator('[data-view="micro"]').click();await frame('phone viewport microscope');assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);await shot('r10-mobile-micro');
...
await page.locator('#panelbtn').click();await page.locator('#reset').click();await page.locator('#closepanel').click();await frame('mobile reset returns whole specimen');await shot('r10-mobile-hero');
...
await page.goto('http://127.0.0.1:8798/workbenches/landscape-surface-r9/index.html');await page.waitForFunction(()=>window.__LM_READY__===true,{},{timeout:240000});await page.locator('[data-view=cliff]').click();await frame('R9 reference cliff','be5944d7');await shot('r9-reference-cliff');

...
async function frame(label,expected='a91a59e3'){
...
await page.goto('http://127.0.0.1:8798/workbenches/landscape-surface-r8/index.html#micro');
...
const gpuDerivative=await require('../landscape-surface-r7/gpu_derivatives.cjs')(page);checks.push('production GPU frame derivative finite difference');console.log('PASS GPU derivatives',gpuDerivative);
...
await frame('micro default');await shot('r8-micro');const defaultCrop=await crop();
await page.evaluate(()=>__LM__.setMaterial({scope:0,micro:0}));await frame('both detail controls off');await shot('r8-all-off');const offCrop=await crop();
await page.evaluate(()=>__LM__.setMaterial({scope:1,micro:0}));await frame('microscope independent of legacy micro switch');await shot('r8-microscope-only');const scopeCrop=await crop();assert(!scopeCrop.equals(offCrop));checks.push('visible microscope-only image difference inside canvas');
...
await page.mouse.move(550,420);await page.mouse.wheel(0,-1450);await page.waitForTimeout(250);assert((await page.evaluate(()=>__LM__.state.radius))<.5);await frame('sub-metre zoom');await shot('r8-ultra');
await page.mouse.wheel(0,-10000);await page.waitForTimeout(250);assert.equal(await page.evaluate(()=>__LM__.state.radius),.12);await frame('closest allowed zoom');await shot('r8-minimum-zoom');
await page.evaluate(s=>__LM__.restoreState(s),focus.state);const before=await page.evaluate(()=>__LM__.state.theta);
await page.mouse.move(550,420);await page.mouse.down();await page.mouse.move(640,440,{steps:8});await page.mouse.up();await page.waitForTimeout(250);assert.notEqual(await page.evaluate(()=>__LM__.state.theta),before);await frame('near-surface mouse orbit');await shot('r8-micro-side');
...
for(const view of ['hero','cliff','cave','foot','back','stone','section']){await page.locator('[data-view="'+view+'"]').click();await frame(view);await shot('r8-'+view);}
await page.setViewportSize({width:390,height:844});await page.locator('[data-view="micro"]').click();await frame('phone viewport microscope');assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);await shot('r8-mobile-micro');
...
await page.locator('#panelbtn').click();await page.locator('#reset').click();await page.locator('#closepanel').click();await frame('mobile reset returns whole specimen');await shot('r8-mobile-hero');
await page.setViewportSize({width:1100,height:780});await page.evaluate(()=>__LM__.build({schema:'landscape-function-world/1',core:'limestone-water-2',seed:211,stage:4,fracture:1,relief:1}));await page.locator('[data-view="micro"]').click();const otherFP=await page.evaluate(()=>__LM__.bufferFingerprint());assert.notEqual(otherFP,'a91a59e3');assert(await page.evaluate(()=>!!__LM__.focus));await frame('another seed has real surface focus',otherFP);
await page.goto('http://127.0.0.1:8798/workbenches/landscape-surface-r7/index.html');await page.waitForFunction(()=>window.__LM_READY__===true,{},{timeout:240000});await page.locator('[data-view=cliff]').click();await frame('R7 reference cliff','42a73c10');await shot('r7-reference-cliff');
assert.equal(errors.length,0);
const report={date:new Date().toISOString().slice(0,10),htmlSHA256:crypto.createHash('sha256').update(fs.readFileSync(path.join(__dirname,'index.html'))).digest('hex'),browser:'Chromium headless / SwiftShader',realHTTP:true,physicalPhoneTested:false,hardwarePerformanceApproved:false,geometryFingerprint:'a91a59e3',focusTriangle:focus.focus,gpuDerivative,checks,frames,errors,visualApproved:false,productionReady:false};

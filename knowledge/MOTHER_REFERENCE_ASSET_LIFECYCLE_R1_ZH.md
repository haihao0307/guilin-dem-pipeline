...
KAOPU 的根本目标不是复制某个模型、某张图或某个作者的表达，而是学习、理解并用统一数值重新描述真实世界。

人和 AI 要在同一套可测量尺度、同一物理、同一逻辑和同一语言下理解对象：

- “大 / 小”必须落到明确单位、参照系和测量定义；
- “快 / 慢”必须落到时间、距离、介质和坐标系；
- “热 / 冷”必须落到温度与测量条件；
- “形状 / 生长 / 运动 / 材质 / 行为”必须尽可能由可复核的自然观察和科学数据约束；
- 未知就是未知，不用看起来合理的猜测填空。

因此 KAOPU 的统一语言是：
**自然事实 → 测量 → 统一尺度与坐标 → 函数/关系 → 独立生成 → 反向验证。**

第三方模型、动画和参考艺术只允许作为过渡学习工具；最终生产体系必须逐步回到自然本身，并在移除这些过渡参考后仍能独立成立。

“道法自然”不是审美口号，而是所有 Mother 的最高数据原则和验收方向。

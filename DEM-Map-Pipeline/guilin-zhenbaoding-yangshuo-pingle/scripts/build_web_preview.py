...
class TerrainRenderer{
 constructor(canvas,meta,height,mask){this.canvas=canvas;this.globalMeta=meta;this.meta=meta;this.height=height;this.mask=mask;this.yaw=.72;this.pitch=.62;this.distance=2.7;this.target=[0,.05,0];this.exaggeration=1;this.drag=false;this.ecology=null;this.gaea={erosion:.25,deposition:.25,surface:.30,talus:.18,karst:.25,rock:.22,vegetation:.45,water:.70};this.focused=false;this.init();this.initLabels();this.bind();this.resize();this.loadEcology();requestAnimationFrame(()=>this.draw())}
 ...
 init(){const g=this.gl=this.canvas.getContext('webgl2',{antialias:true});if(!g)throw new Error('WebGL2 unavailable');
  ...
  const vao=g.createVertexArray();g.bindVertexArray(vao);const vb=g.createBuffer();g.bindBuffer(g.ARRAY_BUFFER,vb);g.bufferData(g.ARRAY_BUFFER,verts,g.STATIC_DRAW);const pl=g.getAttribLocation(this.programTerrain,'p'),hl=g.getAttribLocation(this.programTerrain,'h');g.enableVertexAttribArray(pl);g.vertexAttribPointer(pl,3,g.FLOAT,false,16,0);g.enableVertexAttribArray(hl);g.vertexAttribPointer(hl,1,g.FLOAT,false,16,12);const ib=g.createBuffer();g.bindBuffer(g.ELEMENT_ARRAY_BUFFER,ib);g.bufferData(g.ELEMENT_ARRAY_BUFFER,idx,g.STATIC_DRAW);this.vao=vao;this.vp=g.getUniformLocation(this.programTerrain,'vp');
   const wvs=`#version 300 es\nin vec3 p;uniform mat4 vp;void main(){gl_Position=vp*vec4(p,1.);}`,wfs=`#version 300 es\nprecision highp float;uniform vec4 waterFx;out vec4 o;void main(){vec2 q=gl_FragCoord.xy*.014;float ripple=.5+.5*sin(q.x+q.y);vec3 base=mix(vec3(.035,.24,.31),vec3(.08,.48,.62),waterFx.x);base=mix(base,vec3(.16,.35,.36),waterFx.y*.35);base+=ripple*.018*waterFx.z;o=vec4(base,.78+.18*waterFx.w);}`;this.programWater=this.program(wvs,wfs);this.waterFx=g.getUniformLocation(this.programWater,'waterFx');const water=[];for(const poly of this.meta.waterwayPolygons||[]){for(const tri of triangulateWater(poly.points||[])){for(const point of tri)water.push(...this.waterPoint(point[0],point[1]))}}this.waterCount=water.length/3;this.waterVao=g.createVertexArray();g.bindVertexArray(this.waterVao);const wb=g.createBuffer();g.bindBuffer(g.ARRAY_BUFFER,wb);g.bufferData(g.ARRAY_BUFFER,new Float32Array(water),g.STATIC_DRAW);const wp=g.getAttribLocation(this.programWater,'p');g.enableVertexAttribArray(wp);g.vertexAttribPointer(wp,3,g.FLOAT,false,12,0);this.waterVp=g.getUniformLocation(this.programWater,'vp');g.enable(g.DEPTH_TEST);g.enable(g.BLEND);g.blendFunc(g.SRC_ALPHA,g.ONE_MINUS_SRC_ALPHA)}
 ...
 resize(){const d=Math.min(devicePixelRatio||1,2),w=Math.max(1,this.canvas.clientWidth),h=Math.max(1,this.canvas.clientHeight);if(this.canvas.width!==Math.round(w*d)||this.canvas.height!==Math.round(h*d)){this.canvas.width=Math.round(w*d);this.canvas.height=Math.round(h*d);this.gl.viewport(0,0,this.canvas.width,this.canvas.height)}}
  draw(){const g=this.gl;this.resize();g.clearColor(.006,.014,.012,1);g.clear(g.COLOR_BUFFER_BIT|g.DEPTH_BUFFER_BIT);const cp=Math.cos(this.pitch),eye=[this.target[0]+Math.sin(this.yaw)*cp*this.distance,this.target[1]+Math.sin(this.pitch)*this.distance+.12,this.target[2]+Math.cos(this.yaw)*cp*this.distance],vp=mul(perspective(.74,this.canvas.width/this.canvas.height,.0005,30),lookAt(eye,this.target,[0,1,0]));g.useProgram(this.programTerrain);g.bindVertexArray(this.vao);g.uniformMatrix4fv(this.vp,false,vp);g.uniform1f(this.ex,this.exaggeration);g.uniform4f(this.fx,this.gaea.erosion,this.gaea.surface,this.gaea.karst,this.gaea.vegetation);g.uniform4f(this.fx2,this.gaea.talus,this.gaea.vegetation,this.gaea.rock,this.gaea.deposition);g.drawElements(g.TRIANGLES,this.count,g.UNSIGNED_INT,0);if(this.waterCount){g.useProgram(this.programWater);g.bindVertexArray(this.waterVao);g.uniformMatrix4fv(this.waterVp,false,vp);g.uniform4f(this.waterFx,this.gaea.water,this.gaea.deposition,this.gaea.surface,this.gaea.water);g.drawArrays(g.TRIANGLES,0,this.waterCount)}if(this.ecology?.count&&this.distance<.72){g.useProgram(this.programEcology);g.bindVertexArray(this.ecologyVao);g.uniformMatrix4fv(this.ecologyVp,false,vp);g.uniform1f(this.ecologyZoom,Math.min(1,Math.max(0,(.72-this.distance)/.55)));g.drawArrays(g.POINTS,0,this.ecology.count)}this.updateLabels(vp);requestAnimationFrame(()=>this.draw())}
}

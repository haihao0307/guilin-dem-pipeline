...
const DROP_CODES=new Set([1,2,21]); // motorway, motorway_link, construction
const MAJOR_BRIDGE_CODES=new Set([3,4,5,6,7,8]); // trunk/primary/secondary + links only
const CLASS_NAMES={1:'motorway',2:'motorway_link',3:'trunk',4:'trunk_link',5:'primary',6:'primary_link',7:'secondary',8:'secondary_link',9:'tertiary',10:'tertiary_link',21:'construction'};
...
function filterRoadParts(buffer){
  ...
  let motorway=0,motorwayLink=0,construction=0,majorBridge=0,removedSegments=0,keptVertexRefs=0;
  for(let off=0;off<buffer.byteLength;off+=16){
    ...
    if(DROP_CODES.has(code)){
      remove=true;
      if(code===1)motorway++;else if(code===2)motorwayLink++;else construction++;
  ...
  return{buffer:out,motorway,motorwayLink,construction,majorBridge,majorBridgeByClass,removedSegments,keptParts:kept.length/4,keptVertexRefs};
}
function decorateUi(summary){
  const apply=()=>{
    ...
    if(canvas){
      ...
      canvas.dataset.historyRemovedConstructionParts=String(summary.construction);
...
export function installHistory1942Preclean(){
  ...
  const manifestPromise=(async()=>{
    ...
    const summary={motorway:0,motorwayLink:0,construction:0,majorBridge:0,majorBridgeByClass:{},removedParts:0,removedSegments:0,patches:[]};
    for(const patch of manifest.patches||[]){
      ...
      delete patch.roads.classPartCounts.motorway;delete patch.roads.classPartCounts.motorway_link;delete patch.roads.classPartCounts.construction;
      ...
      patch.roads.history1942Preclean={removedMotorwayParts:f.motorway,removedMotorwayLinkParts:f.motorwayLink,removedConstructionParts:f.construction,removedMajorBridgeParts:f.majorBridge,removedMajorBridgeByClass:f.majorBridgeByClass,removedSegments:f.removedSegments};
      summary.motorway+=f.motorway;summary.motorwayLink+=f.motorwayLink;summary.construction+=f.construction;summary.majorBridge+=f.majorBridge;summary.removedSegments+=f.removedSegments;
      ...
      summary.patches.push({id:patch.id,motorway:f.motorway,motorwayLink:f.motorwayLink,construction:f.construction,majorBridge:f.majorBridge,majorBridgeByClass:f.majorBridgeByClass,removedParts:f.motorway+f.motorwayLink+f.construction+f.majorBridge});
    }
    summary.removedParts=summary.motorway+summary.motorwayLink+summary.construction+summary.majorBridge;
    manifest.history1942Preclean={schema:'wenzhou-history-1942-preclean/r3',active:true,controlSource1953:'records/R3_10/HISTORICAL_1953_CONTROL_SOURCE.json',removedClasses:['motorway','motorway_link','construction'],removedBridgePolicy:{flag:'bridge',majorRoadClasses:['trunk','trunk_link','primary','primary_link','secondary','secondary_link'],minorBridgeClassesPreserved:true},railwayAction:'none-current-runtime-has-no-railway-layer',deferred:['tertiary and lower local bridges','ordinary roads','buildings','reservoirs and dams','reclamation and historical coastline','port hard shoreline'],summary};

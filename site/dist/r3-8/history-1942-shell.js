(async()=>{
  ...
  const identity=`<script>window.__WENZHOU_HISTORY_1942=true;(function(){var p=document.querySelector('.brand p');if(p)p.textContent='1940s Map Mother · 历史海陆回退';document.documentElement.dataset.wenzhouMapMotherEpoch='1940s';var location=document.getElementById('location');if(location)location.value='overview';var soil=document.getElementById('show-soil-context');if(soil)soil.checked=false;var buildings=document.getElementById('show-osm-buildings');if(buildings)buildings.checked=false;})();</script>`;

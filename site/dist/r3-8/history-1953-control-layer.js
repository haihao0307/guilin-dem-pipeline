...
function applyHistoricalLandMask(scene,terrain,masks){
  ...
  terrain.userData.wenzhouMapMotherHistoryAlphaMap=masks.texture;terrain.userData.wenzhouMapMother1940s=true;
  terrain.material.alphaMap=masks.texture;terrain.material.alphaTest=Math.max(.5,Number(terrain.material.alphaTest||0));terrain.material.needsUpdate=true;
  let seaUpdated=false;scene.traverse(object=>{if(object.userData?.wenzhouSeaDemo&&object.material?.uniforms?.uLandMask){object.material.uniforms.uLandMask.value=masks.texture;object.material.needsUpdate=true;seaUpdated=true;}});
  return seaUpdated;
}

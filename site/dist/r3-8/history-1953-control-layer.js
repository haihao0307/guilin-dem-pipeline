...
function isTerrainCandidate(object){return !!(object?.isMesh&&!object.userData?.wenzhouSeaDemo&&!object.userData?.wenzhouSurfaceEvidence&&!object.userData?.wenzhouLandcoverEvidence&&!object.userData?.wenzhouOsmEvidence&&!object.userData?.wenzhouHistory1953&&object.material?.alphaMap&&object.geometry?.attributes?.position);}
...
function terrainSampler(terrain){
  ...
  return function sample(x,z){const i=lowerCell(xs,x),j=lowerCell(zs,z);if(i<0||j<0||!valid[j*(nc-1)+i])return null;const x0=xs[i],x1=xs[i+1],z0v=zs[j],z1=zs[j+1];if(x1===x0||z1===z0v)return null;const u=(x-x0)/(x1-x0),v=(z-z0v)/(z1-z0v);const a=pos.getY(j*nc+i)*sy,b=pos.getY(j*nc+i+1)*sy,c=pos.getY((j+1)*nc+i)*sy,d=pos.getY((j+1)*nc+i+1)*sy;return u+v<=1?a+(b-a)*u+(c-a)*v:d+(c-d)*(1-u)+(b-d)*(1-v);};
}
function utm51(lonDeg,latDeg){
  const a=6378137,f=1/298.257223563,e2=f*(2-f),ep2=e2/(1-e2),k0=.9996,lon0=123*Math.PI/180;
  const lat=latDeg*Math.PI/180,lon=lonDeg*Math.PI/180,s=Math.sin(lat),c=Math.cos(lat),t=Math.tan(lat),N=a/Math.sqrt(1-e2*s*s),T=t*t,C=ep2*c*c,A=c*(lon-lon0);
  ...
  const NN=k0*(M+N*t*(A*A/2+(5-T+9*C+4*C*C)*A**4/24+(61-58*T+T*T+600*C-330*ep2)*A**6/720));
  return[E,NN];
}
...
function ensureUi(){
  if(document.getElementById('show-history-1953-control'))return;
  const host=document.querySelector('.base-layers .hydro-options')||document.querySelector('.focus-panel');if(!host)return;
  const label=document.createElement('label');label.innerHTML='<input id="show-history-1953-control" type="checkbox"> 1953 历史水域/岸线控制';host.appendChild(label);
  const panel=document.querySelector('.focus-panel');if(panel){const card=document.createElement('div');card.id='history-1953-layer-card';card.className='osm-card';card.hidden=true;card.innerHTML='<strong>1953 历史控制层</strong><span id="history-1953-layer-status">等待地形…</span><small>由用户提供的 1953 L500 图幅提取并地理配准。只用于大尺度海岸、开阔水域、岛屿、围垦/水库冲突判断；不是测绘级岸线。</small>';panel.prepend(card);}
}
export function installHistorical1953ControlLayer(){
  ...
  THREE.Object3D.prototype.add=function(...objects){const result=originalAdd.apply(this,objects);for(const object of objects){if(isTerrainCandidate(object)){ensureUi();queueMicrotask(()=>build(this,object));}}return result;};
  const bind=()=>{ensureUi();document.getElementById('show-history-1953-control')?.addEventListener('change',setVisible);};if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',bind,{once:true});else bind();
}

...
function refineMask(terrain,box){
  ...
  const {protect,seeds}=buildHighGroundProtection(terrain,base,w,h,box),xc=utm51(...XUANMEN_CENTER);let xuanmenOpened=0,islandRestored=0;
...
function applyTexture(scene,terrain,refined){
  const old=terrain.userData.wenzhouMapMotherRefinedAlphaMap;if(old&&old!==refined.texture)old.dispose?.();terrain.userData.wenzhouMapMotherRefinedAlphaMap=refined.texture;terrain.material.alphaMap=refined.texture;terrain.material.alphaTest=.42;terrain.material.needsUpdate=true;let seaUpdated=false;

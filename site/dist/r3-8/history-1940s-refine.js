...
function buildHighGroundProtection(terrain,baseData,w,h,box){
  const protect=new Uint8Array(w*h),pos=terrain.geometry?.attributes?.position;if(!pos)return{protect,seeds:0,outsideBaseSeeds:0};const scaleY=terrain.scale?.y??1;let seeds=0,outsideBaseSeeds=0;
  for(let i=0;i<pos.count;i++){
    const hm=pos.getY(i)*scaleY*1000;if(hm<HIGH_GROUND_M)continue;const[px,py]=pixelFromLocal(box,w,h,pos.getX(i),pos.getZ(i));if(px<0||py<0||px>=w||py>=h)continue;const q=(py*w+px)*4;seeds++;if(baseData[q+1]<128)outsideBaseSeeds++;
    for(let dy=-2;dy<=2;dy++)for(let dx=-2;dx<=2;dx++){if(dx*dx+dy*dy>6)continue;const x=px+dx,y=py+dy;if(x>=0&&y>=0&&x<w&&y<h)protect[y*w+x]=1;}
  }
  return{protect,seeds,outsideBaseSeeds};
}
...
function refineMask(terrain,box){
  ...
  const {protect,seeds,outsideBaseSeeds}=buildHighGroundProtection(terrain,base,w,h,box),xc=utm51(...XUANMEN_CENTER);let xuanmenOpened=0,islandRestored=0,islandOutsideBaseRestored=0;
  ...
  for(let p=0;p<protect.length;p++)if(protect[p]){const q=p*4;if(img[q+1]<128){const outsideBase=base[q+1]<128;img[q]=img[q+1]=img[q+2]=255;img[q+3]=255;islandRestored++;if(outsideBase)islandOutsideBaseRestored++;}}
  ...
  return{texture,data:smooth,width:w,height:h,xuanmenOpened,islandRestored,islandOutsideBaseRestored,highGroundSeeds:seeds,highGroundOutsideBaseSeeds:outsideBaseSeeds,landGrowthClamped};
}

...
const FLAG=Symbol.for('wenzhou.r3.8.soil-context-installed');
...
function fetchJson(url){return fetch(url).then(r=>{if(!r.ok)throw Error(`R3.8 JSON读取失败 (${r.status})`);return r.json();});}
async function checkedBytes(url,expectedSha,expectedBytes,signal){
  const r=await fetch(url,{signal});if(!r.ok)throw Error(`R3.8 SoilGrids读取失败 (${r.status})`);
  const b=await r.arrayBuffer();
  if(b.byteLength!==expectedBytes)throw Error(`R3.8 SoilGrids字节数不一致 ${b.byteLength} != ${expectedBytes}`);
  if(!crypto.subtle)throw Error('当前浏览器不能执行 R3.8 SHA-256 证据校验');
  const actual=Array.from(new Uint8Array(await crypto.subtle.digest('SHA-256',b)),v=>v.toString(16).padStart(2,'0')).join('');
  if(actual!==expectedSha)throw Error(`R3.8 SoilGrids SHA-256 不一致: ${actual}`);
  return b;
}
export function terrainOrigin(contract,patch){
...
export function isTerrainCandidate(object){
...
function layerFor(manifest,prop,stat){
  const layer=manifest.layers.find(x=>x.property===prop&&x.statistic===stat&&x.depth===(document.getElementById('soil-depth')?.value||'0-5cm'));
  if(!layer)throw Error(`R3.8 缺少 SoilGrids ${prop} ${stat}`);
  return layer;
}
...
function makeNormalizedTexture(raw,layer){
  ...
  if(!Number.isFinite(lo)||!Number.isFinite(hi)||hi<=lo)throw Error(`R3.8 ${layer.property} ${layer.statistic} percentile range invalid`);
...
export function installSoilContext(){
  ...
  const manifestPromise=fetchJson(MANIFEST_URL).then(m=>{
    if(m.schema!=='wenzhou-r3.8-soil-profile/r1'||m.sourceIdentity!=='model_prediction_external_observation'||m.resolutionM!==250||m.truthBoundary?.canonicalTruth!==false||m.truthBoundary?.mayReplaceFieldSurvey!==false||m.truthBoundary?.mayOverrideCanonicalDem!==false||m.displayPolicy?.heightDisplacement!==false||m.displayPolicy?.uncertaintyIsRelativeIndex!==true)throw Error('R3.8 SoilGrids manifest evidence boundary mismatch');
    return m;
  });
  const contractPromise=fetchJson(TERRAIN_URL);
  let abort=null;const captured=new WeakSet();
  ...
  function captureRenderer(terrain,scene){if(captured.has(terrain))return;captured.add(terrain);const prior=terrain.onAfterRender;terrain.onAfterRender=function(renderer,renderScene,camera,...rest){activeRenderer=renderer;activeScene=scene;activeCamera=camera;if(typeof prior==='function')prior.call(this,renderer,renderScene,camera,...rest);};}
  ...
  async function loadLayer(layer,signal){
    const key=layer.sha256;
    if(cache.has(key))return cache.get(key);
    const b=await checkedBytes(new URL(layer.path,DATA_BASE),layer.sha256,layer.bytes,signal);
    const raw=new Int16Array(b);
    if(raw.length!==layer.rows*layer.columns)throw Error(`R3.8 ${layer.property} ${layer.statistic} shape 不一致`);
    cache.set(key,raw);while(cache.size>4)cache.delete(cache.keys().next().value);return raw;
  }
  ...
  function soilMode(){return (document.getElementById('evidence-mode')?.value||'soil')==='soil';}
  function overlayEnabled(){return soilMode()&&(document.getElementById('show-soil-context')?.checked??true);}
  function updateCard(manifest,median,unc,medianRaw,uncRaw,contract,patchId){
    ...
    status.textContent=`250 m 模型预测 · ${median.depth} · ${median.statistic}`;
  ...
  async function buildForTerrain(scene,terrain){
    const token=++buildToken;abort?.abort();abort=new AbortController();const signal=abort.signal;activeTerrain=terrain;activeScene=scene;dispose();if(!soilMode()){setDataset({soilContextLoaded:false,soilContextVisible:false});return;}captureRenderer(terrain,scene);await new Promise(r=>requestAnimationFrame(r));
    const patchId=canvas()?.dataset.patch||document.getElementById('location')?.value||'';
    setDataset({soilContextLoaded:false,soilContextVisible:false,soilContextPatch:patchId,soilContextError:null});document.getElementById('soil-retry').hidden=true;document.getElementById('soil-context-status').textContent='读取并校验当前深度…';document.getElementById('soil-context-value').textContent='';document.getElementById('soil-context-uncertainty').textContent='';
    try{
      const[manifest,contract]=await Promise.all([manifestPromise,contractPromise]);if(token!==buildToken)return;
      const patch=contract.patches.find(p=>p.id===patchId);if(!patch)throw Error(`R3.8 缺少地形视域 ${patchId}`);
      const prop=selectedProperty(),median=layerFor(manifest,prop,'Q0.5'),unc=layerFor(manifest,prop,'uncertainty');
      const[medianRaw,uncRaw]=await Promise.all([loadLayer(median,signal),loadLayer(unc,signal)]);if(token!==buildToken)return;
      ...
      setDataset({
        ...
        soilContextResolutionM:manifest.resolutionM,soilContextDepth:median.depth,soilContextMedianSha256:median.sha256,soilContextUncertaintySha256:unc.sha256,soilContextUncertaintyVisible:uncertaintyEnabled(),
    ...
    }catch(error){if(token!==buildToken||error.name==='AbortError')return;dispose();setDataset({soilContextKind:'error',soilContextLoaded:false,soilContextVisible:false,soilContextPatch:patchId,soilContextError:error.message});const card=document.getElementById('soil-context-card');if(card)card.hidden=false;document.getElementById('soil-context-title').textContent='土壤证据暂不可用';document.getElementById('soil-context-status').textContent=error.message;document.getElementById('soil-retry').hidden=false;}
  }
  ...
  document.getElementById('soil-retry').addEventListener('click',()=>{if(activeScene&&activeTerrain)void buildForTerrain(activeScene,activeTerrain);});
  const show=document.getElementById('show-soil-context');if(show&&!show.dataset.boundSoilContext){show.dataset.boundSoilContext='true';show.addEventListener('change',()=>{if(active)active.mesh.visible=show.checked&&soilMode();setDataset({soilContextVisible:!!active&&show.checked&&soilMode()});forceRender();});}
  for(const id of ['soil-depth','evidence-mode'])document.getElementById(id)?.addEventListener('change',()=>{document.getElementById('soil-context-card').hidden=!soilMode();if(activeScene&&activeTerrain)void buildForTerrain(activeScene,activeTerrain);});

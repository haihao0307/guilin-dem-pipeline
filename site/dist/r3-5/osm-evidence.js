...
function makeBuildingSegments(xy,parts,meta,origin,sampler){
  ...
  const positions=[];let drawnRings=0,drawnSegments=0,rejectedSegments=0;
  for(let p=0;p<parts.length;p+=4){
    const start=parts[p],count=parts[p+1];if(count<2)continue;let ringDrawn=false;
    for(let i=0;i<count-1;i++){
      ...
      positions.push(x0,h0+BUILDING_LIFT_M/1000,z0,x1,h1+BUILDING_LIFT_M/1000,z1);drawnSegments++;ringDrawn=true;
    }
    if(ringDrawn)drawnRings++;
  }
  ...
  const line=new THREE.LineSegments(g,m);line.renderOrder=2.4;line.userData.wenzhouOsmEvidence=true;line.userData.kind='osm-building-footprint-boundary-evidence';line.userData.heightClaim='unknown-not-generated';line.userData.extrusionClaim='none';
  return{object:line,drawnRings,drawnSegments,rejectedSegments};
}
...
export function installOsmObjectEvidence(){
  ...
  const manifestPromise=fetchJson(MANIFEST_URL).then(m=>{
    if(m.sourceReleaseAssetSha256!==SOURCE_RELEASE_SHA256||m.sourceIdentity!=='external_mapped_observation'||m.canonicalTruth!==false||m.surveyGradeGeometry!==false||m.individualPhysicalTruth!==false||m.productionReady!==false)throw Error('R3.5 OSM manifest evidence boundary mismatch');
    return m;
  });
  ...
  function updateCard(meta,roads,buildings){
    ...
    document.getElementById('osm-status').textContent=`${meta.id} · 道路线段 ${roads?.drawnSegments?.toLocaleString?.()||0} · 建筑边界段 ${buildings?.drawnSegments?.toLocaleString?.()||0}`;
    document.getElementById('osm-road-count').textContent=`中心线 parts ${meta.roads.partCount.toLocaleString()}`;
    document.getElementById('osm-building-count').textContent=meta.id==='overview'?'全域不载入建筑':`footprint rings ${meta.buildings.ringCount.toLocaleString()}`;
    document.getElementById('osm-source').textContent='道路屏幕线宽不代表物理宽度；建筑只画平面 footprint 边界，不生成高度。© OpenStreetMap contributors · ODbL';
  }
  ...
  async function buildForTerrain(scene,terrain){
    ...
    try{
      ...
      setDataset({
        ...
        osmBuildingRings:meta.buildings.ringCount,osmBuildingSegmentsDrawn:buildings?.drawnSegments||0,osmBuildingSegmentsRejectedNoSurface:buildings?.rejectedSegments||0,
        osmRoadWidthClaim:'none-screen-style-only',osmBuildingHeightClaim:'unknown-not-generated',osmSurfaceAnchor:'current-display-triangles',
        osmSourceReleaseSha256:SOURCE_RELEASE_SHA256,osmQuantizationMaxStepM:meta.quantization.maxStepM,osmLoaded:true,osmError:null
      });forceRender();

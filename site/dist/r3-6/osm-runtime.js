...
async function checkedBytes(url,expectedSha,expectedBytes,signal){
  ...
  catch(error){if(isAbort(error))fetchAbortCount++;throw error;}
  if(!r.ok)throw Error(`R3.6 对象证据读取失败 (${r.status})`);
  let b;
  ...
  throwIfAborted(signal);
  if(actual!==expectedSha)throw Error(`R3.6 对象证据 SHA-256 不一致: ${actual}`);
  return b;
}
...
function terrainSampler(terrain){
  ...
  if(!pos||!index)throw Error('R3.6 无法建立显示曲面锚定：地形缺少 position/index');
...
async function checkpoint(state,signal,force=false){
  ...
  if(force||chunk>=CPU_YIELD_BUDGET_MS){state.yieldCount++;await new Promise(r=>setTimeout(r,0));throwIfAborted(signal);state.chunkStart=performance.now();}
}
function closeCpuState(state){state.maxChunkMs=Math.max(state.maxChunkMs,performance.now()-state.chunkStart);return state;}

async function buildIndexedRoads(xy,parts,meta,origin,sampler,signal){
  if(parts.length%4)throw Error('R3.6 road parts 格式错误');
  ...
  for(let p=0;p<parts.length;p+=4){
    throwIfAborted(signal);
    const start=Number(parts[p]),count=Number(parts[p+1]),code=Number(parts[p+2]),flags=Number(parts[p+3]);if(count<2)continue;
    const color=roadColor(code,flags);let partDrawn=false;
    for(let j=0;j<count;j++){
      const vi=start+j,a=vi*2;if(a+1>=xy.length)throw Error('R3.6 road xy 索引越界');
  ...
  await checkpoint(cpu,signal,true);closeCpuState(cpu);throwIfAborted(signal);
  const exactIndices=indices.slice(0,iw),g=new THREE.BufferGeometry();
  g.setAttribute('position',new THREE.BufferAttribute(positions,3));g.setAttribute('color',new THREE.BufferAttribute(colors,3));g.setIndex(new THREE.BufferAttribute(exactIndices,1));
  const m=new THREE.LineBasicMaterial({vertexColors:true,transparent:true,opacity:.78,depthWrite:false});
  const line=new THREE.LineSegments(g,m);line.frustumCulled=false;line.renderOrder=2.5;line.userData.wenzhouOsmEvidence=true;line.userData.kind='osm-road-centerline-evidence';line.userData.heightClaim='display-surface-anchor-only';line.userData.physicalWidthClaim='none';line.userData.runtime='indexed-r36';
...
async function buildIndexedBuildings(xy,parts,meta,origin,sampler,signal){
  if(parts.length%4)throw Error('R3.6 building parts 格式错误');
  const t0=performance.now(),vertexCount=xy.length/2;
  ...
  for(let p=0;p<parts.length;p+=4){
    throwIfAborted(signal);
    const start=Number(parts[p]),count=Number(parts[p+1]);if(count<2)continue;let partDrawn=false;
    for(let j=0;j<count;j++){
...
export function installOptimizedOsmRuntime(){
  ...
  function updateCard(meta,roads,buildings,totalBuildMs){
    ...
    document.getElementById('osm-title').textContent='OpenStreetMap · R3.6 索引化映射证据';
    document.getElementById('osm-status').textContent=`${meta.id} · 道路 ${roads?.drawnSegments?.toLocaleString?.()||0} 段 · 建筑 ${buildings?.drawnSegments?.toLocaleString?.()||0} 段 · 构建 ${totalBuildMs.toFixed(1)} ms`;
    document.getElementById('osm-road-count').textContent=`道路顶点 ${roads.sourceVertexCount.toLocaleString()} · index ${roads.gpuIndexCount.toLocaleString()}`;
    document.getElementById('osm-building-count').textContent=meta.id==='overview'?'全域不载入建筑':`建筑顶点 ${(buildings?.sourceVertexCount||0).toLocaleString()} · index ${(buildings?.gpuIndexCount||0).toLocaleString()}`;
    document.getElementById('osm-source').textContent='证据与 R3.5 完全相同；R3.6 只改变运行时组织：每个源顶点采样/上传一次，index 连边，过时请求可取消。道路线宽仍非物理宽度，建筑仍无生成高度。';
  }
  ...
  async function loadPayload(meta,signal){
    const cached=payloadCache.get(meta.id);if(cached){cacheHits++;touchCache(meta.id,cached);return cached;}
    cacheMisses++;const f=meta.files;
    const tasks=[typedFile(f.roadXY,Uint16Array,signal),typedFile(f.roadParts,Uint32Array,signal)];
    if(f.buildingXY&&f.buildingParts)tasks.push(typedFile(f.buildingXY,Uint16Array,signal),typedFile(f.buildingParts,Uint32Array,signal));
    const values=await Promise.all(tasks);throwIfAborted(signal);
    const payload={roadXY:values[0],roadParts:values[1],buildingXY:values[2]||null,buildingParts:values[3]||null};touchCache(meta.id,payload);return payload;
  }
  async function buildForTerrain(scene,terrain){
    ...
    try{
      ...
      const payload=await loadPayload(meta,signal);throwIfAborted(signal);if(token!==buildToken)throw abortError('superseded-token');
      const sampler=terrainSampler(terrain),origin=terrainOrigin(contract,patch);
      const roads=await buildIndexedRoads(payload.roadXY,payload.roadParts,meta,origin,sampler,signal);throwIfAborted(signal);
      let buildings=null;
      if(payload.buildingXY&&payload.buildingParts)buildings=await buildIndexedBuildings(payload.buildingXY,payload.buildingParts,meta,origin,sampler,signal);
      ...
      const totalBuildMs=performance.now()-buildStart,maxChunkMs=Math.max(roads.maxChunkMs,buildings?.maxChunkMs||0),yieldCount=roads.yieldCount+(buildings?.yieldCount||0);
      active={scene,roads,buildings,meta,terrainUuid:terrain.uuid};updateCard(meta,roads,buildings,totalBuildMs);
      setDataset({
        ...
        osmSourceReleaseSha256:SOURCE_RELEASE_SHA256,osmCorrectedReportSha256:CORRECTED_REPORT_SHA256,osmQuantizationMaxStepM:meta.quantization.maxStepM,
        osmRuntime:'indexed-r36',osmIndexed:true,osmAbortController:true,osmAbortCount:abortCount,osmFetchAbortCount:fetchAbortCount,osmCacheHits:cacheHits,osmCacheMisses:cacheMisses,
        osmBuildMs:totalBuildMs.toFixed(3),osmMaxChunkMs:maxChunkMs.toFixed(3),osmYieldCount:yieldCount,
        osmRoadSourceVertices:roads.sourceVertexCount,osmRoadGpuVertices:roads.gpuVertexCount,osmRoadGpuIndexCount:roads.gpuIndexCount,osmRoadSampleCalls:roads.sampleCalls,osmRoadIndexType:roads.indexType,
        osmBuildingSourceVertices:buildings?.sourceVertexCount||0,osmBuildingGpuVertices:buildings?.gpuVertexCount||0,osmBuildingGpuIndexCount:buildings?.gpuIndexCount||0,osmBuildingSampleCalls:buildings?.sampleCalls||0,osmBuildingIndexType:buildings?.indexType||'none',
        osmLoading:false,osmLoaded:true,osmError:null
      });forceRender();
    }catch(error){
      if(isAbort(error)){if(token===buildToken)setDataset({osmLoading:false,osmLoaded:false,osmAbortCount:abortCount,osmFetchAbortCount:fetchAbortCount});return;}
      if(token!==buildToken)return;disposeActive();hideCard();setDataset({osmKind:'error',osmPatch:patchId,osmLoading:false,osmLoaded:false,osmAbortCount:abortCount,osmFetchAbortCount:fetchAbortCount,osmError:error.message});console.error(error);
    }
  }

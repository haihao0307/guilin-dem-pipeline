...
const { createFieldContext, evaluatePoint } = window.LandscapeMotherFieldEvaluator;
...
function deriveFields(denseTruth, grid, spacing, subdivision, manifest, contract, segments) {
  ...
  const context = createFieldContext(
    denseTruth, grid, spacing, subdivision, manifest, contract, segments, blurs,
  );
  ...
  for (let row = 0; row < grid; row += subdivision) {
    for (let column = 0; column < grid; column += subdivision) {
      ...
      sourceNodeMaxError = Math.max(
        sourceNodeMaxError,
        Math.abs(arrays.enhanced[index] - denseTruth[index]),
      );
    }
  }
  ...
  return {
    ...
    fields: {
      ...
      paddy: arrays.paddy,
      wet: arrays.wet,
      alluvium: arrays.alluvium,
      bund: arrays.bund,
      ditch: arrays.ditch,
      fracture: arrays.fracture,
      strata: arrays.strata,
      flow: arrays.flow,
      sediment: arrays.sediment,
      unitSeed: arrays.unitSeed,
    },
    receipt: {
      surfaceRangeM: [stats.surfaceMin, stats.surfaceMax],
      fieldRangeM: [stats.fieldMin, stats.fieldMax],
      ...
      peakShiftM,
      paddyCoverage: stats.paddySum / denseTruth.length,
    },
  };
}

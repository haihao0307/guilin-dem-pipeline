/* Weather Mother 1.1.0. Data-only periodic evolution and measured performance.
...
(function(root){'use strict';
 ...
 class FrameStats{
  constructor(gl){this.gl=gl;this.ext=gl.getExtension('EXT_disjoint_timer_query_webgl2');this.queue=[];this.active=null;this.gpu=[];this.cpu=[];this.intervals=[];this.lastComplete=null;this.frames=0;this.disjointSamples=0;this.beginCpu=0;this.epoch=0;}
  ...
  end(){this.cpu.push(performance.now()-this.beginCpu);if(this.cpu.length>120)this.cpu.shift();if(this.active){this.gl.endQuery(this.ext.TIME_ELAPSED_EXT);this.queue.push({query:this.active,epoch:this.epoch});this.active=null;}}
  ...
  reset(){this.epoch++;this.lastComplete=null;this.intervals=[];this.gpu=[];this.cpu=[];}
  poll(){if(!this.ext)return;const g=this.gl;if(g.getParameter(this.ext.GPU_DISJOINT_EXT)){for(const q of this.queue)g.deleteQuery(q.query);this.disjointSamples+=this.queue.length;this.queue=[];this.gpu=[];return;}while(this.queue.length&&g.getQueryParameter(this.queue[0].query,g.QUERY_RESULT_AVAILABLE)){const item=this.queue.shift(),q=item.query,ms=g.getQueryParameter(q,g.QUERY_RESULT)/1e6;g.deleteQuery(q);if(item.epoch===this.epoch&&Number.isFinite(ms)&&ms>0){this.gpu.push(ms);if(this.gpu.length>120)this.gpu.shift();}}}

...
result={'version':'0.6.2-loop','measurementRevision':2,'status':'RUNNING','scope':'SwiftShader comparison only; not user hardware or AAA performance approval','publicURL':BASE,'windows':[],'storedImageFiles':0,'visualAcceptance':False,'aaaQualityApproved':False,'userHardwarePerformanceVerified':False}
...
try:
 ...
 with sync_playwright() as p:
  ...
  page.goto(BASE+'?ab=062-r2',wait_until='domcontentloaded',timeout=60000)
  ...
  for enabled in [False,True,True,False,False,True]:
   ...
   start=page.evaluate('(()=>{WeatherMother.resetMeasurements();return WeatherMother.qa.frames})()')
   page.wait_for_function('(n)=>WeatherMother.qa.errors.length>0||(WeatherMother.qa.frames>=n+8&&WeatherMother.qa.performance.samples>=6)',arg=start,timeout=120000)
   q=page.evaluate('WeatherMother.qa');assert not q['errors'],q['errors'];assert q['frames']>=start+8
   st=page.evaluate('WeatherMother.getState()');assert st['timeScale']==0
   item={'fastEmpty':enabled,'freshWindowFrameStart':start,'freshWindowFrameEnd':q['frames'],'performance':q['performance'],'renderSize':q['renderSize'],'raySteps':q['steps'],'seed':st['seed'],'kind':st['kind'],'phase':st['loopPhase'],'simTime':q['simulationTimeS']}
 ...
 result['meanTimeReductionPercent']=(off-on)/off*100;result['equalQualityAndScene']=True;result['freshWindowsVerified']=True;result['status']='MEASURED'
 result['interpretation']='Single software-renderer experiment. No statistically significant or user-hardware speedup claim is made.'
except Exception:
 import traceback

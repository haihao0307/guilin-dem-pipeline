...
function occupancyField(data,dims,lo,hi){
 ...
 const distance=occupancyDistance(a,size);return{data:a,distance,size,radius,occupied,total:a.length,detailWarpBoundKm:.56};
}
...
}else if(c.kind==='Cu'||c.kind==='Cb'){const pos=[[0,0,1],[-6.6,-1.5,.71],[6.0,-3.2,.76],[-6.0,-7.0,.58],[7.,3.9,.59],[.8,-8.,.61],[-10.2,4.,.43],[10.4,-6.7,.45],[1.,7.,.44]];for(let k=0;k<c.count;k++){let[x,z,s]=pos[k];cumulus(x+(random()-.5)*.6,z+(random()-.5)*.6,s,c.kind==='Cb'&&k===0);}}
...
self.postMessage({id:c.id,kind:c.kind,data:out,groups,lobes:lobes.length,borderMax:border,supportSafe,seed:c.seed,spacing,tau,shadowSize,occupancy},[out.buffer,tau.buffer,occupancy.data.buffer,occupancy.distance.buffer]);

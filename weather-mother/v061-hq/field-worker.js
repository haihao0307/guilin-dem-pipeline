...
function group(start){let lo=[Infinity,Infinity,Infinity],hi=[-Infinity,-Infinity,-Infinity];for(const a of lobes.slice(start)){const r=[Math.max(a[3],a[5]),a[4],Math.max(a[3],a[5])];for(let k=0;k<3;k++){lo[k]=Math.min(lo[k],a[k]-r[k]-2.6);hi[k]=Math.max(hi[k],a[k]+r[k]+2.6);}}groups.push({lo,hi});}

...
// Source-field optical-depth proxy, with detailed contact attenuation.
// This is a bounded graphical multiple-scattering approximation, not calibrated radiometry.
float studioDepth(vec3 p,vec3 L,float size){vec3 tangent=normalize(cross(L,abs(L.y)>.97?vec3(1,0,0):vec3(0,1,0))),bitangent=cross(L,tangent);float tau=(densityAt(p+L*.025,true)+densityAt(p+L*.075,false))*.05*2.4;for(int j=0;j<12;j++){float a=float(j)/12.,b=float(j+1)/12.,nearD=.10+6.*a*a,farD=.10+6.*b*b,ds=farD-nearD,t=(farD+nearD)*.5,angle=float(j)*2.399963;vec3 cone=(cos(angle)*tangent+sin(angle)*bitangent)*(t*tan(size)*.5);float b0=shape(p+L*t+cone-uWind);tau+=smoothstep(.018,.21,b0)*uOpt.x*.85*ds*2.4;}return tau;}
vec3 studioIllumination(vec3 p,vec3 rd){float h=smoothstep(1.2,4.4,p.y-uWind.y);vec3 result=mix(vec3(.008,.013,.024),vec3(.016,.024,.037),h);for(int k=0;k<3;k++){if(lightPower[k]<=0.)continue;vec3 L=normalize(lightDir[k]);float tau=studioDepth(p,L,lightSize[k]),mu=dot(rd,L);float ph=.72*phaseHG(.64,mu)+.28*phaseHG(-.18,mu);float direct=exp(-tau),multiple=.050*exp(-tau*.50)+.015*exp(-tau*.15);result+=lightColor[k]*lightPower[k]*(direct*(.25+.12*min(ph,5.))+multiple);}return result;}

...
const detail=new URL('./detail-overlay.mjs',base).href;
let code=text.replace("'./source-bundle.mjs'",JSON.stringify(bundle));
code=code.replace("'./detail-overlay.mjs'",JSON.stringify(detail));
if(code===text)throw new Error('runtime import marker missing');

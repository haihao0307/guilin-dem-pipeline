...
const url = process.env.QA_CANDIDATE_URL || 'http://127.0.0.1:4173/apps/ocean-life-mother/fish-mother/yellowfin-biological-correction-r001/candidate-a-qa.html';
const outDir = path.resolve(process.env.QA_CANDIDATE_OUT || 'apps/ocean-life-mother/fish-mother/yellowfin-biological-correction-r001/evidence/candidate-a-browser');
const viewport = { width: 1440, height: 900 };
const receiptPath = path.join(outDir, 'CANDIDATE_A_BROWSER_QA_RECEIPT.json');
...
function buildReceipt({ qa, screenshots, consoleErrors, pageErrors, fatalError = null }) {
  ...
  return {
    schema: 'kaopu.fish-mother.yellowfin-biological-correction-candidate-a-browser-qa/1.0',
    date: '2026-09-21',
    build: 'YELLOWFIN-BIOLOGICAL-CORRECTION-R001-CANDIDATE-A-BROWSER-QA',
    url,
    ...
    candidateAnimations: qa?.candidateAnimations || [],
...
async function main() {
  ...
  try {
    ...
    if (!initial.ready) {
      await snap('candidate-a-qa-failure.png');
    ...
    await snap('candidate-a-workbench-ui.png');
    await page.evaluate(() => {
    ...
    await snap('candidate-a-frozen-side-rest.png');
    await selectMode('candidate');
    await snap('candidate-a-corrected-side-rest.png');
    await selectMode('overlay');
    await snap('candidate-a-overlay-side-rest.png');

    await selectMode('candidate');
    for (const view of ['quarter', 'top', 'front', 'dorsal', 'pectoral']) {
      await selectView(view);
      await snap(`candidate-a-corrected-${view}-rest.png`);
    }

    await selectMode('overlay');
    await selectView('dorsal');
    await snap('candidate-a-overlay-dorsal-rest.png');
    await selectView('pectoral');
    await snap('candidate-a-overlay-pectoral-rest.png');

    for (const ratio of [0.25, 0.5, 0.75, 0.999]) {
      await selectMode('overlay');
      await selectView('side');
      await selectTimeRatio(ratio);
      await snap(`candidate-a-overlay-side-t${String(Math.round(ratio * 1000)).padStart(3, '0')}.png`);
    }

    await selectMode('candidate');
    await selectTimeRatio(0.5);
    await selectView('quarter');
    await snap('candidate-a-corrected-quarter-tmid.png');
    await selectView('top');
    await snap('candidate-a-corrected-top-tmid.png');
    ...
    fs.writeFileSync(receiptPath, `${JSON.stringify(receipt, null, 2)}\n`);
    if (!receipt.passed) {
      throw new Error(`Candidate A browser QA failed: ${JSON.stringify({ failedChecks: receipt.failedChecks, consoleErrors, pageErrors })}`);
    }
    process.stdout.write(`${JSON.stringify(receipt, null, 2)}\n`);
  } catch (error) {
    if (!fs.existsSync(receiptPath)) {
      try { await snap('candidate-a-qa-failure.png'); } catch {}
      let qa = {};

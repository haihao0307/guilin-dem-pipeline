...
${common}
uniform vec3 origin;uniform float objScale,yaw,phase,bodyH,bodyW,tailH,dorsal;uniform int kind,palette,shape;
...
float t=clamp((-.44-p.x)/.38,0.,1.);float h=mix(.024,tailH,pow(t,.9));float fork=(shape==2||shape==5||palette==5)?.11:0.;float tailEnd=-.82+fork*(1.-clamp(abs(p.y)/max(tailH,.01),0.,1.));d=max(max(abs(p.z)-.009,abs(p.y)-h),max(p.x+.42,tailEnd-p.x));res=hitmin(res,vec2(d,1.));

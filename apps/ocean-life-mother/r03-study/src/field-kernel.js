...
function fish(doc,plan=1){
 ...
 for(const p of doc.patches){const sampled=sampleCoordinates(p,plan),detail=sampled.detail,dvStep=1/Math.max(1,sampled.H-1);let boundarySamples=0;
  for(const [u,v,u0,u1] of sampled.coords){const a=patchAt(p,u,v);if(a.some(x=>!Number.isFinite(x)))throw Error('Nonfinite generated field');
   ...
   const ul=Math.max(u0,u-duStep),ur=Math.min(u1,u+duStep),vd=clamp(v-dvStep,0,1),vu=clamp(v+dvStep,0,1);
   const left=patchAt(p,ul,v),right=patchAt(p,ur,v),down=patchAt(p,u,vd),up=patchAt(p,u,vu);
   const du=Math.hypot(...right.slice(0,3).map((e,i)=>(e-left[i])*.5)),dv=Math.hypot(...up.slice(0,3).map((e,i)=>(e-down[i])*.5));const radius=clamp(Math.max(du,dv)*.78,.00023,.018);if(Math.abs(u-u0)<1e-12||Math.abs(u-u1)<1e-12)boundarySamples++;
  ...
  patchPlans.push({id:p.id,H:sampled.H,detail,parameterSamples:sampled.coords.length,boundarySamples,rows:sampled.rows.length});
 }
 return{data:new Float32Array(out),count:validCount,stride:18,patchPlans,detail:patchPlans.length?patchPlans[0].detail:1,canonical:'separable continuous scalar fields; domain-aware direct generated sample cache',sourceTriangleCount:0,intermediateRaster:false,boundaryPreserving:true};
}
return{decode,field,patchAt,curve,valid,fish,reconstruct,sample,conduct,resolution,domainSpans,sampleCoordinates};})();

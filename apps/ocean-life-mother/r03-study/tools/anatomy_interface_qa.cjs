...
const spec={schema:'kaopu-fish-anatomy-articulation/0.1',coordinateFrame:'source-local',lengthUnit:'source-unit',angleUnit:'rad',parts:[
...
test('missing anchor rejected instead of zero default',()=>{assert.throws(()=>A.compile({schema:spec.schema,coordinateFrame:spec.coordinateFrame,lengthUnit:spec.lengthUnit,angleUnit:spec.angleUnit,parts:[{id:'bad',kind:'rigid-hinge',parent:null,axis:[1,0,0],angleRange:[0,1]}]}),/anchor/);return true});
test('missing frame or units rejected',()=>{assert.throws(()=>A.compile({schema:spec.schema,parts:[]}),/frame and units/);return true});
test('hierarchy cycle rejected',()=>{assert.throws(()=>A.compile({schema:spec.schema,coordinateFrame:spec.coordinateFrame,lengthUnit:spec.lengthUnit,angleUnit:spec.angleUnit,parts:[{id:'a',kind:'rigid-fixed',parent:'b',anchor:[0,0,0]},{id:'b',kind:'rigid-fixed',parent:'a',anchor:[0,0,0]}]}),/cycle/);return true});

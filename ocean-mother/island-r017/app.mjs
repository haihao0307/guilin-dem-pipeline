...
function decorateDeep(){
 ...
 doc.documentElement.dataset.oceanGlassSkin='r017';
...
function updateParticles(dt,time){
 ...
 while(smokeEmit>=1){spawnSmoke();smokeEmit--}while(flameEmit>=1){spawnFlame();flameEmit--}while(sprayEmit>=1){spawnSpray(time);sprayEmit--}while(emberEmit>=1){spawnEmber();emberEmit--}
...
async function init(){
 ...
 window.OceanIsland=window.OceanCoast=window.OceanCoastR012=api;loading.classList.add('done');lastFrame=performance.now();requestAnimationFrame(frame);
}
init().catch(fail);

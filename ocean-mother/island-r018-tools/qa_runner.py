...
with sync_playwright() as p:
 ...
 if page.evaluate('!!window.OceanIsland?.qa.ready'):
  ...
  state=page.evaluate('OceanIsland.getState()');details['initial']=state;check('correct_identity',state['version']=='0.3.0-island-r018')
  ...
  check('r018_relationship_flags',page.evaluate("OceanIsland.qa.shallowWaterLayering&&OceanIsland.qa.foamBandCoupling&&OceanIsland.qa.waterFireReflection&&OceanIsland.qa.smokeCoolingCurve"))
  check('three_band_state_exposed',page.evaluate("OceanIsland.sampleWater(36,0,0).bandStrengths.length===3&&OceanIsland.sampleWater(36,0,0).bandOffsets.length===3"))
  ...
  if URL.startswith('https:'):
   ...
   check('deep_glass_skin_applied',page.evaluate("document.getElementById('deepFrame').contentDocument.documentElement.dataset.oceanGlassSkin==='r018'"))

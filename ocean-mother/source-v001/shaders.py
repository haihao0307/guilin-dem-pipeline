...
void main(){float d=.45*(exp(grid.y*11.10)-1.);vec3 center=camera+flatForward*d;vec2 xz=center.xz+right.xz*(grid.x*2.-1.)*(max(16.,d*1.48));vec3 p=vec3(xz.x,0.,xz.y),dx=vec3(1,0,0),dz=vec3(0,0,1);float rowSpan=max(.06,(d+.45)*11.10/gridRows),c=0.;
...
''',
'water.frag':r'''precision highp float;
...
vec3 col=reflection*F+water*(1.-F)+min(spec,vec3(18.));float froth=0.;if(doFoam>.5){float patch=noise2(wPos.xz*1.5+tSea*.09)*.62+noise2(wPos.xz*5.7-tSea*.13)*.38;float white=(1.-smoothstep(.01,.28,compression-.51-foamGain*.22))*smoothstep(.38,.72,patch);float breakup=noise2(wPos.xz*22.);froth=white*foamGain*(.4+.6*breakup)*(1.-smoothstep(700.,1900.,dist));col=mix(col,vec3(.73,.83,.83)*(.15+.85*uDay)*(.55+.45*transmission),clamp(froth,0.,.85));}
...
O=vec4(film(col),1.);}
''',
'sky.frag':r'''precision highp float;
...
''',
'cloud-atlas.frag':r'''uniform float encodedEnv;

...
uniform mat4 uShadowVP;uniform sampler2D uSolidShadow;
...
float solidShadow(vec3 p,vec3 n){vec4 q=uShadowVP*vec4(p+n*.016,1.);vec3 v=q.xyz/q.w*.5+.5;if(any(lessThan(v,vec3(0)))||any(greaterThan(v,vec3(1))))return 1.;float s=0.;for(int x=-1;x<=1;x++)for(int y=-1;y<=1;y++){float z=texture(uSolidShadow,v.xy+vec2(x,y)/2048.).r;s+=step(v.z-.00012,z);}return s/9.;}
vec3 direct(vec3 p,vec3 n,vec3 albedo){vec3 light;if(uMode==0.)light=vec3(.17,.23,.28)*(.08+.92*uDay)+uSunColor*max(dot(n,uSun),0.)*1.2*solidShadow(p,n)*step(0.,uSun.y);else if(uMode==1.)light=vec3(.24)+vec3(.82)*max(dot(n,normalize(vec3(-.5,.8,.3))),0.)*solidShadow(p,n);else{light=vec3(.08,.09,.11);for(int k=0;k<3;k++)light+=uLightColor[k]*uLightPower[k]*max(dot(n,uLightDir[k]),0.)*(k==0?solidShadow(p,n):1.);}
...
if(tag<.5){float ripple=sin(P.z*10.+sin(P.x*1.4)*1.8);albedo=mix(vec3(.35,.30,.22),vec3(.63,.56,.43),b);albedo*=.97+.045*grain+.012*ripple;n=normalize(n+vec3(.008*cos(P.x*12.),0,.025*cos(P.z*10.)));albedo*=mix(1.,.52,wet);}
else if(tag<1.5){float mineral=fb(P*3.7),grain2=noise3(P*35.);albedo=mix(vec3(.115,.105,.087),vec3(.285,.26,.215),b);albedo=mix(albedo,vec3(.38,.34,.27),smoothstep(.71,.83,mineral)*.20);albedo*=.93+.11*grain2;n=normalize(n+vec3((noise3(P*13.)-.5)*.09,(grain-.5)*.04,(noise3(P*13.+31.)-.5)*.09));albedo*=mix(1.,.62,wet);}
...
vec3 col=direct(P,n,albedo);vec3 v=normalize(uEye-P),h=normalize(v+uSun);float spec=pow(max(dot(n,h),0.),mix(12.,105.,wet));col+=uSunColor*spec*wet*.09*solidShadow(P,n)*step(0.,uSun.y);if(tag>2.5)col+=vec3(1.5,.08,.002)*pow(max(0.,.6-noise3(P*7.)),3.)*uHeat*2.;O=vec4(col,1);}`;
export const waterFS=`#version 300 es
__COMMON__
in vec3 P,N;in vec4 D;out vec4 O;
void main(){if(D.y<.006||insideRock(P))discard;if(D.w< -100.){vec3 c=mix(vec3(.014,.18,.20),vec3(.006,.048,.065),sat(-P.y/5.));O=vec4(uMode==3.?vec3(.05,.22,.3):c*(.65+.35*uDay),1.);return;}float shallow=sat(D.y/.3),a=uMicro*shallow;vec2 p=P.xz;vec2 slope=vec2(0.);for(int k=0;k<7;k++){float j=float(k),f=1.4*pow(1.67,j),ang=j*2.399;vec2 d=vec2(cos(ang),sin(ang));float amp=.058*pow(.57,j);slope+=d*amp*f*cos(dot(p,d)*f-uTime*sqrt(9.81*f)+j*4.7);}
vec3 n=normalize(N+vec3(slope.x,0,slope.y)*a*2.);vec3 v=normalize(uEye-P);if(dot(n,v)<0.)n=-n;float fres=.02+.98*pow(1.-max(dot(n,v),0.),5.);vec3 reflected=skyLight(reflect(-v,n));float path=min(25.,D.y/max(abs(v.y),.17));vec3 tr=exp(-vec3(.72,.18,.10)*path);float sandNoise=fb(vec3(p*.75,2.));vec3 bottom=mix(vec3(.27,.21,.12),vec3(.45,.37,.23),sandNoise);vec3 body=mix(vec3(.004,.065,.092),bottom,tr);body*=(.55+.45*uDay)*(.50+.50*solidShadow(P,n));
vec3 col=mix(body,reflected,fres);vec3 h=normalize(v+uSun);float shin=pow(max(dot(n,h),0.),150.);col+=uSunColor*shin*.85*solidShadow(P,n)*step(0.,uSun.y);

...
report={'status':'RUNNING','url':URL,'checks':[],'errors':[],'visualApproved':False,'productionApproved':False,'userHardwarePerformanceVerified':False,'source':'real Chromium browser','performanceScope':'software-rendered paused views; no hardware speed claim','version':'0.1.2-coast'}
...
try:
 with sync_playwright() as pw:
  ...
  check('domain dimensions and runtime version match',page.evaluate("OceanCoast.qa.version==='0.1.2-coast'&&OceanCoast.qa.domainMetres[0]===96&&OceanCoast.qa.domainMetres[1]===84"))
  def image(label):
  ...
  # Regression: a long interaction frame must no longer dead-stop the simulation.

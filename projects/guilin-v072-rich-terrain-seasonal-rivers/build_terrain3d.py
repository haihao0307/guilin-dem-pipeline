...
def _prepare_runs(ds, hydrology: dict[str, Any], step: float):
    ...
    for feature_index, feature in enumerate(hydrology.get("features", [])):
        ...
        for section in _contiguous(common):
            ...
            re, rn = e[section], n[section]; run_ne, run_nn = _line_normals(re, rn)
            run = {"source_feature_index": feature_index, "emitted_run_index": emitted, "run_id": f"f{feature_index}:r{emitted}",
                ...
                "normal_eastings": run_ne, "normal_northings": run_nn}
            runs.append(run)
...
def _geometry_from_triangle_polygons(triangles: np.ndarray, coverage_valid: bool | None = None) -> Any:
    if not len(triangles):
        return Polygon()
    if bool(coverage_is_valid(triangles)) if coverage_valid is None else coverage_valid:
        try:
...
def _decoded_run_topology(run: dict[str, Any], positions: np.ndarray, indices: np.ndarray) -> tuple[dict[str, Any], Any, dict[Any, Any]]:
    ...
    geometry = _geometry_from_triangle_polygons(triangle_values, coverage_valid)
...
def _cross_run_contract(geometries: list[Any], precomputed_union: Any | None = None) -> dict[str, Any]:
    ...
    summed_area = float(sum(geometry.area for geometry in visible))
    union = precomputed_union if precomputed_union is not None else _union_parts(visible)
...
def _display_precision_contract(raw_desired: Any, display_owned: Any,
    ...
    result = {
        ...
        "raw_desired_union": _topology(raw_desired),
        ...
        "measurement_scope": "independent raw q16 desired union versus fixed 1/64m globally-noded displayed ownership",
    }
    checks = {
...
def _season_bank_asset(ds, output_dir: Path, runs: list[dict[str, Any]], season: str,
    ...
    for run in runs:
        ...
        normal_e, normal_n = run["normal_eastings"], run["normal_northings"]
        half = run["base_width_m"] * preset["width"] / 2
...
def _build_river_season(ds, output_dir: Path, runs: list[dict[str, Any]], junctions: dict[str, Point],
    ...
    print(f"RIVER_SEASON_START season={season} runs={len(runs)}", flush=True)
    for run in runs:
    ...
    # Raw precision is a reference union only.  Unique per-run ownership is proved by
    # the fixed-grid partition below, which is also the geometry used by the renderer.
    # Running a second raw atomic-face partition produced no final mesh input and made
    # the production build repeat its most expensive global noding pass four times.
    raw_desired_union = _clean_geometry(unary_union(candidates))
    print(f"RIVER_SEASON_RAW_UNION season={season} elapsed_s={time.monotonic() - started:.3f}", flush=True)
    ...
    display_contract = _display_precision_contract(raw_desired_union, display_owned_union, display_partition.contract(junctions))
    print(f"RIVER_SEASON_DISPLAY_PARTITION season={season} elapsed_s={time.monotonic() - started:.3f}", flush=True)
    del display_partition, candidates, display_candidates, raw_desired_union
    run_ranges = []
    ...
    for run_index, (run, geometry) in enumerate(zip(runs, owned, strict=True)):
        if run_index % 100 == 0:
            print(f"RIVER_SEASON_TERRAIN season={season} run={run_index}/{len(runs)} elapsed_s={time.monotonic() - started:.3f}", flush=True)
    ...
    cross_serialized = _cross_run_contract(serialized_by_run, serialized_union)
    cross_expected = _cross_run_contract(expected_by_run, expected_union)
    ...
    preclip_symmetric = float(display_owned_union.symmetric_difference(accounted_union).area)
    serialized_over_nodata = float(serialized_union.intersection(nodata_union).area)
    serialized_over_extent = float(serialized_union.intersection(extent_union).area)
    ...
    checks = {
        ...
        "nodata_and_extent_remain_transparent": serialized_over_nodata <= max(1e-6, nodata_union.boundary.length * RIVER_FLOAT32_XZ_TOLERANCE_M) and serialized_over_extent <= max(1e-6, extent_union.boundary.length * RIVER_FLOAT32_XZ_TOLERANCE_M),
        "positive_run_count": len(run_ranges) > 0,
        "preclip_owned_continuously_accounted_after_terrain_clipping": preclip_delta["hausdorff_m"] <= RIVER_FLOAT32_XZ_TOLERANCE_M and preclip_delta["first_outside_second_buffer_length_m"] <= RIVER_BOUNDARY_LENGTH_EPSILON_M and preclip_delta["second_outside_first_buffer_length_m"] <= RIVER_BOUNDARY_LENGTH_EPSILON_M and preclip_symmetric <= numerical_tolerance,
    ...
    final = {
        ...
        "serialized_over_nodata_area_m2": serialized_over_nodata,
        "serialized_over_extent_clipped_area_m2": serialized_over_extent,
        ...
        "preclip_owned_to_accounted_after_terrain_clipping_symmetric_difference_area_m2": preclip_symmetric,
    ...
    print(f"RIVER_SEASON_COMPLETE season={season} elapsed_s={final['duration_seconds']:.3f} vertices={decoded_vertex_count} indices={decoded_index_count}", flush=True)

...
def classify_geomorphic(name: str | None) -> str:
...
def join_single(points: gpd.GeoDataFrame, polygons: gpd.GeoDataFrame, fields: list[str], prefix: str) -> gpd.GeoDataFrame:
    ...
    if "index_right" in out.columns:
        out = out.sort_values(["_sound_id", "index_right"], kind="stable").drop_duplicates("_sound_id", keep="first")
...
def main() -> None:
    ...
    snd["semantic_zone"] = snd.get("ALLEN_class_name", pd.Series(index=snd.index, dtype=object)).map(classify_geomorphic)
    snd["allen_mapped"] = snd.get("ALLEN_class_name", pd.Series(index=snd.index, dtype=object)).notna()
    ...
    osm = load(osm_path)
    osm_reef = osm[osm.get("semanticRole", pd.Series(index=osm.index, dtype=object)).astype(str).eq("reef")].copy()
    polytypes = {"Polygon", "MultiPolygon"}
    osm_reef = osm_reef[osm_reef.geometry.geom_type.isin(polytypes)]
    if len(osm_reef):
    ...
    for zone in ZONE_CODE:
        ...
        zone_summary[zone] = {
            "allenClassCounts": {str(k): int(v) for k, v in grp.get("ALLEN_class_name", pd.Series(dtype=object)).fillna("UNMAPPED").astype(str).value_counts().items()},
            "depthStatsM": depth_stats(grp["depth_m"]),
            "catzocCounts": {str(k): int(v) for k, v in grp.get("zoc_label", pd.Series(dtype=object)).fillna("UNQUALIFIED").astype(str).value_counts().items()},
            "byCATZOC": by_zoc,
            "insideOsmReefPolygon": int(grp["inside_osm_reef_polygon"].sum()),
        }

    high_quality = snd[snd.get("zoc_label", pd.Series(index=snd.index, dtype=object)).astype(str).isin(["A1", "A2", "B"])].copy()
    allen_mapped_hq = high_quality[high_quality["allen_mapped"]].copy()
    unmapped_hq = high_quality[~high_quality["allen_mapped"]].copy()

    # Rasterize semantic zones on the exact R02 depth-grid geometry. This is a traceability
    # companion grid only; it does not overwrite depth or imply a bathymetric value.
    ...
    arr = rasterize(shapes, out_shape=(h, w), transform=transform, fill=ZONE_CODE["unclassified_water"], dtype="uint8")
    ...
    tab = out.drop(columns="geometry").copy()
    tab["lon"] = out.geometry.x
    tab["lat"] = out.geometry.y
    ...
    depare_vertical_counts = Counter(str(v) for v in snd.get(depare_verdat_col, pd.Series(dtype=object)).dropna().astype(str)) if depare_verdat_col else Counter()
    ...
    report = {
        "schema": "kaopu.palau.reef-interior-zone-reconciliation-r08/1.0",
        "coreBBoxWGS84": CORE,
        "sourceIdentity": {
            "soundings": {"path": str(sound_path.relative_to(results)), "sha256": sha256(sound_path), "role": "measured ENC sounding evidence with M_QUAL lineage"},
            "allenGeomorphic": {"path": str(allen_path.relative_to(results)), "sha256": sha256(allen_path), "role": "shallow geomorphic semantic evidence, not depth truth"},
            "depare": {"path": str(depare_path.relative_to(results)), "sha256": sha256(depare_path), "role": "ENC charted depth-area context"},
            "osm": {"path": str(osm_path.relative_to(results)), "sha256": sha256(osm_path), "role": "independent community semantic cross-check, not hydrographic truth"},
        },
        "verticalDatum": {
    ...
    if ip.exists():
        ...
        if marker not in html:
            section = (
                ...
                '<p>Allen/OSM 只提供语义边界；没有把浅水地貌强制转换为深度，也没有改变 R02 水深栅格。R08 新增同网格语义分区用于 PalauWorld.sample() 的证据追踪。</p></section>'
            )

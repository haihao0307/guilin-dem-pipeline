"""Unmodified original vs actual integrated canvas; identical view/time, UI hidden for A/B only."""
from pathlib import Path
from playwright.sync_api import sync_playwright
from http.server import ThreadingHTTPServer,SimpleHTTPRequestHandler
from functools import partial
from PIL import Image,ImageChops,ImageStat
import threading,json,hashlib,traceback,signal
...
try:
 with sync_playwright() as p:
  ...
  for name,uri in [('original','source/v0160/frozen/original-deep-v001.html?still'),('integrated','releases/v0.1.6.0/Stone_Money_Island_V0.1.6.0_Direct_Open.html?reference')]:
   ...
   page.goto('http://127.0.0.1:8766/'+uri,wait_until='domcontentloaded');page.add_style_tag(content='body > :not(canvas):not(script):not(style){visibility:hidden!important}')
   if name=='original':

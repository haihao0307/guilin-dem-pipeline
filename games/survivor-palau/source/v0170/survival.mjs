...
const SURVIVAL={
 ...
 observation:{active:false,anchorX:0,anchorZ:0,bobY:0,focusX:0,focusZ:0,breath:1,fog:.08,leak:.03,calm:.12,ingress:0,snorkelClearanceM:.2,lowProfile:false,actionDisturbance:0,lastYaw:0,lastPitch:0,geometryClock:0,uiClock:0},
 ...
 fish:[],
 telemetry:{visibleFishCount:0,cameraSurfaceOffsetM:0,snorkelSubmerged:false,fishRespondToMotion:false,lineEndDepthM:0,historyDateResolved:true,visualAcceptance:false,productionReady:false}
};
...
function enterSurfaceObservation(point=FISH.target){
 const o=SURVIVAL.observation;if(o.active&&point){o.focusX=point.x;o.focusZ=point.z;return true}
 ...
 const fx=Math.sin(CANOE_STATE.yaw),fz=Math.cos(CANOE_STATE.yaw);o.focusX=point?.x??o.anchorX+fx*7.2;o.focusZ=point?.z??o.anchorZ+fz*7.2;o.active=true;o.lowProfile=false;o.breath=Math.max(o.breath,.72);o.calm=.10;o.actionDisturbance=.45;o.lastYaw=camera.yaw;o.lastPitch=camera.pitch;
...
function pilotUpdateObservation(dt){
 const o=SURVIVAL.observation,w=waveAt(o.anchorX,o.anchorZ,physicalTime,config).eta;o.bobY=mix(o.bobY,w,pilot01(dt*2.0));
 const moveRate=(Math.abs(pilotAngleDelta(camera.yaw,o.lastYaw))+Math.abs(camera.pitch-o.lastPitch))/Math.max(.016,dt);o.lastYaw=camera.yaw;o.lastPitch=camera.pitch;const motion=moveRate*.22+Math.abs(CANOE_STATE.speed)*.8+o.actionDisturbance;o.actionDisturbance=Math.max(0,o.actionDisturbance-dt*.72);
 ...
 o.fog=pilot01(o.fog+dt*(.006+.012*o.leak));o.calm=pilot01(o.calm+dt*((motion<.30&&o.ingress<.25) ? .18 : -.70));
 ...
 pilotUpdateFish(dt,motion);return motion;
}
...
function updatePilotSurvival(dt){
 ...
 pilotUpdateObservation(dt);o.geometryClock+=dt;o.uiClock+=dt;let changed=false;

"""Real-browser gate for Stone Money Island V0.2.7."""
...
EVIDENCE=Path('/tmp/stone-money-v0270-qa');EVIDENCE.mkdir(parents=True,exist_ok=True)
ENTRY='releases/v0.2.7/index.html'
server=ThreadingHTTPServer(('127.0.0.1',8770),partial(SimpleHTTPRequestHandler,directory=str(ROOT)))
threading.Thread(target=server.serve_forever,daemon=True).start()
report={'version':'0.2.7','browserPassed':False,'visualAcceptance':False,'physicalDeviceTest':False,'cases':[]}
...
def neutral(path,box):return fraction(path,box,lambda r,g,b:max(r,g,b)-min(r,g,b)<18 and 60<(r+g+b)/3<205)
def green(path,box):return fraction(path,box,lambda r,g,b:g>r*1.18 and g>b*1.08 and g>42)
...
with sync_playwright() as p:
 ...
 for name,width,height in [('desktop',960,540),('mobile',390,844)]:
  ...
  try:
   page.goto('http://127.0.0.1:8770/'+ENTRY,wait_until='domcontentloaded',timeout=60000)
   ...
   assert life['diag']['archGeometry']=='dissolved-mushroom-rock-island-v027'
   assert life['diag']['archOpening']['secondarySeaCave'] is True
   assert life['diag']['archOpening']['offCenter'] is True and life['diag']['archOpening']['tidalUndercut'] is True
   assert abs(life['diag']['archMass']['tidalToCrownWidthRatio']-.46)<.001
   assert life['diag']['archMass']['vegetationPockets']==7 and life['diag']['archMass']['unequalCrownLobes'] is True
   assert life['diag']['archMass']['minimumRoofThickness']>=5.7
   case['life']=life
   ...
   for mode in ['aerial','arch','fish']:
    ...
    if name=='desktop' and mode=='arch':
     cf=chroma(shot,(385,205,590,390));nf=neutral(shot,(260,100,720,360));gf=green(shot,(250,55,720,250));case['archImageMetrics']={'openingChroma':cf,'broadNeutral':nf,'crownGreen':gf};assert cf>.13,cf;assert nf<.79,nf;assert gf>.008,gf

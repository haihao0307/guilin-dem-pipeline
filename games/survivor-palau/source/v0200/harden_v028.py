...
an audited open-water corridor, and places the dedicated review camera inside
that corridor rather than behind foreground islands.
"""
...
def apply(s: str) -> str:
    ...
    # Coordinate audit: [210,190] was actually crowded by authored distant
    # islands at [236,154], [255,181], [224,204] and [154,174].  The landmark
    # is therefore moved to [96,208], whose nearest authored mass is > 65 m
    # away from the centre and outside the arch crown envelope.
    rep("const archBed=Math.max(host.bed(186,144),-8);b.dissolvedArch([186,archBed+7.8,144],31.5,10.6,[.36,.37,.32]);",
        "const archBed=Math.max(host.bed(96,208),-8);b.dissolvedArch([96,archBed+7.8,208],31.5,10.6,[.36,.37,.32]);")

    # Review from a lagoon-height line of sight down the clear approach.  The
    # player is not moved and the camera is inspection-only.
    rep("if(cameraMode==='arch'){camera.eye=[246,32,258];camera.target=[184,10,144];camera.forward=norm(sub(camera.target,camera.eye));camera.right=norm(cross(camera.forward,[0,1,0]));camera.up=cross(camera.right,camera.forward);camera.fov=46*Math.PI/180;host.lookAt(camera.view,camera.eye,camera.target,[0,1,0]);host.perspective(camera.proj,camera.fov,aspect,.15,60000);return true;}",
        "if(cameraMode==='arch'){camera.eye=[101,12.6,270];camera.target=[100,7.8,208];camera.forward=norm(sub(camera.target,camera.eye));camera.right=norm(cross(camera.forward,[0,1,0]));camera.up=cross(camera.right,camera.forward);camera.fov=47*Math.PI/180;host.lookAt(camera.view,camera.eye,camera.target,[0,1,0]);host.perspective(camera.proj,camera.fov,aspect,.15,60000);return true;}")
    ...
    rep("cameraMode,archGeometry:'dissolved-mushroom-rock-island-v027',archOpening:{nominalMainSpan:18.6,height:19.5,tidalUndercut:true,asymmetric:true,offCenter:true,secondarySeaCave:true},archMass:{tidalToCrownWidthRatio:.46,vegetationPockets:7,unequalCrownLobes:true,minimumRoofThickness:5.7},archCrown:'irregular-pocket-vegetation-same-geometry-build',source:",
        "cameraMode,archGeometry:'dissolved-mushroom-rock-island-v028-revealed',archPosition:[96,208],archOpening:{nominalMainSpan:23.94,height:21.19,tidalUndercut:true,asymmetric:true,offCenter:true,secondarySeaCave:true},archMass:{tidalToCrownWidthRatio:.46,vegetationPockets:7,unequalCrownLobes:true,minimumRoofThickness:4.91},archCrown:'irregular-pocket-vegetation-same-geometry-build',source:")

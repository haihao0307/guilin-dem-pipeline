"""Real-browser gate for Stone Money Island V0.2.5."""
...
EVIDENCE=Path('/tmp/stone-money-v0250-qa');EVIDENCE.mkdir(parents=True,exist_ok=True)
ENTRY='releases/v0.2.5/index.html'
server=ThreadingHTTPServer(('127.0.0.1',8768),partial(SimpleHTTPRequestHandler,directory=str(ROOT)))
threading.Thread(target=server.serve_forever,daemon=True).start()
report={'version':'0.2.5','browserPassed':False,'visualAcceptance':False,'physicalDeviceTest':False,'cases':[]}
...
def chroma_fraction(path:Path,box):
 im=Image.open(path).convert('RGB').crop(box);px=list(im.getdata())
...
with sync_playwright() as p:
 ...
 for name,width,height in [('desktop',960,540),('mobile',390,844)]:
  ...
  try:
   page.goto('http://127.0.0.1:8768/'+ENTRY,wait_until='domcontentloaded',timeout=60000)
   ...
   shore=page.evaluate("""() => {
    ...
    for(const t of [0,3,6,9,12]){
      ...
      for(let r=60;r<=120;r+=1){const q=StoneMoneyShoreline.physicalWaterAt(r,0,t);if(q.allowed)wet++;else dry++;if(q.allowed&&(q.physicalDepth<=.008||q.bed>=q.runupCeiling))bad++;}
   ...
   if name=='desktop':
    left=pale_fraction(aerial,(0,270,40,370));right=pale_fraction(aerial,(920,270,960,370));case['aerialPaleEdgeFraction']={'left':left,'right':right};assert left<.25 and right<.25

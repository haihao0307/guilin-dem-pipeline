"""Real-browser gate for Stone Money Island V0.2.8."""
...
EVIDENCE=Path('/tmp/stone-money-v0280-qa');EVIDENCE.mkdir(parents=True,exist_ok=True)
ENTRY='releases/v0.2.8/index.html'
server=ThreadingHTTPServer(('127.0.0.1',8771),partial(SimpleHTTPRequestHandler,directory=str(ROOT)))
threading.Thread(target=server.serve_forever,daemon=True).start()
report={'version':'0.2.8','browserPassed':False,'visualAcceptance':False,'physicalDeviceTest':False,'cases':[]}
...
def neutral(path,box):return fraction(path,box,lambda r,g,b:max(r,g,b)-min(r,g,b)<18 and 55<(r+g+b)/3<205)
...
with sync_playwright() as p:
 ...
 for name,width,height in [('desktop',960,540),('mobile',390,844)]:
  ...
  try:
   page.goto('http://127.0.0.1:8771/'+ENTRY,wait_until='domcontentloaded',timeout=60000)
   ...
   for mode in ['aerial','arch','fish']:
    ...
    if name=='desktop' and mode=='arch':
     cf=chroma(shot,(300,155,660,430));nf=neutral(shot,(230,90,730,440));gf=green(shot,(220,45,760,260));case['archImageMetrics']={'openingChroma':cf,'broadNeutral':nf,'crownGreen':gf};assert cf>.10,cf;assert nf<.82,nf;assert gf>.006,gf

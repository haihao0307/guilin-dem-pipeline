...
OUT=HERE.parents[1]/'releases/v0.2.3';OUT.mkdir(exist_ok=True,parents=True)
...
receipt={'version':'0.2.3','sourceBranch':'work/stone-money-island-v0230-fast-iteration-20260919','baseCommit':'fac8e6c8d1bcc585ab76547822ff07689f55377b','baseOceanSha256':hashlib.sha256(base.read_bytes()).hexdigest(),'entrySha256':hashlib.sha256(s.encode()).hexdigest(),'bytes':len(s.encode()),'frozenShaderAndWorkerStringsUnchanged':True,'qaChangesGeometry':False,'sharedProjectionNear':.15,'shorelineQuery':'StoneMoneyShoreline.physicalWaterAt re-evaluates waveSurface and bedHeight per sample; fragment gate uses the same functions','candidateBeach':{'radius':92,'beachWidth':50,'shelfWidth':84,'nominalAreaRatioVsOriginal':new_area/old_area,'regime':'reef-protected lagoon','surveyed':False},'physicalWaterGate':'fragment-space wave/bed depth and a dynamic run-up ceiling reject water that interpolates through dry sand','fishWaterBinding':'uncaught fish remain water-bound; a Fish inspection camera exposes the current school while Fish Mother R02 structural carrier is reviewed for later source-coupled import','constructedStoneHouseRemoved':True,'curlSheetDrawn':False,'stoneMoneyRockIsland':True,'distantKarstCountAdded':5,'archLandmark':'procedural three-body rock arch','deepTrenchMeters':185,'inspectionViews':['aerial','fish'],'fishMotherReviewed':'R02 native generic 9-bone carrier; not species-accepted','visualAcceptance':False,'physicalDeviceTest':False,'publicVerified':False,'gameplay':['on-foot first person','wooden spear craft and geometric fish hit','coconut use','inventory object transitions','natural shelter rest and day counter','patrol LOS candidate and failure','local save','damaged radio inspection only']}

...
def apply(s: str) -> str:
 ...
 const ga=(u,m,w)=>Math.exp(-Math.pow((u-m)/w,2)),topAt=u=>c[1]+11.8+13.4*ga(u,-.70,.38)+7.3*ga(u,.64,.28)+5.1*ga(u,-.05,.72)-3.1*ga(u,.22,.18)+1.15*Math.sin(u*4.5+.7)+.62*Math.sin(u*10.7-1.0)+.26*Math.sin(u*25.0+.4),baseAt=u=>c[1]-7.6+.70*Math.sin(u*5.1-.3)+.28*Math.sin(u*15.0+1.1),inMain=u=>u>mainL&&u<mainR,inCave=u=>u>caveL&&u<caveR;
 const archCurve=(u,a,b,rise,pow,skew)=>{const mid=(a+b)*.5,half=(b-a)*.5,q=clamp((u-mid)/half,-1,1),d=Math.pow(Math.max(0,1-q*q),pow);return c[1]+1.0+rise*d*(1+skew*q)+.38*Math.sin(u*13.0+.4)*d;},holeAt=u=>inMain(u)?archCurve(u,mainL,mainR,12.7,.58,.18):(inCave(u)?archCurve(u,caveL,caveR,4.6,.72,-.10):baseAt(u)),lowAt=u=>holeAt(u);
...
'''
    ...
    rep(
        ...
        "cameraMode,archGeometry:'dissolved-mushroom-rock-island-v027',archOpening:{nominalMainSpan:18.6,height:21.5,tidalUndercut:true,asymmetric:true,offCenter:true,secondarySeaCave:true},archMass:{tidalToCrownWidthRatio:.46,vegetationPockets:7,unequalCrownLobes:true},archCrown:'irregular-pocket-vegetation-same-geometry-build',source:"
    )

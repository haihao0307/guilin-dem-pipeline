...
const report={schema:'wenzhou-r3.6-inflight-cancel-probe/r2',target,baseline,afterRequestStarted,counters:{started,finished,failed,failedUrls},final,errors,telemetryNote:'fetchAbortCount is diagnostic only; authoritative cancellation evidence is controller abort + browser requestfailed/net::ERR_ABORTED + server CLIENT_ABORT checked by the workflow.',passed:false};
const failures=[];
...
if(!failedUrls.some(x=>/ERR_ABORTED|aborted/i.test(x.error)))failures.push(`browser failures were not explicit aborts: ${JSON.stringify(failedUrls)}`);

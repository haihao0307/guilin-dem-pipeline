...
try{
  ...
  await page.waitForFunction(()=>Number(document.querySelector('#terrain')?.dataset.history1940sMaskWidth||0)>0,{},{timeout:60000});await page.waitForTimeout(800);
  ...
  const mobile=await browser.newContext({viewport:{width:390,height:844},reducedMotion:'reduce'});if(target.includes('githack'))await mobile.setExtraHTTPHeaders({Cookie:'__Http-phish=1'});const mp=await mobile.newPage();await mp.goto(target,{waitUntil:'domcontentloaded',timeout:120000});await mp.waitForFunction(()=>document.documentElement.dataset.wenzhouHistoryMode==='1942-preclean-r3',{},{timeout:120000});await mp.waitForFunction(()=>Number(document.querySelector('#terrain')?.dataset.history1940sMaskWidth||0)>0,{},{timeout:60000});check(await mp.locator('#history-1953-toggle').count()===0,'mobile legacy guide toggle survived');const mbrand=await mp.locator('.brand p').textContent();check(mbrand?.includes('1940s Map Mother'),'mobile Map Mother label missing');await mp.screenshot({path:`${out}/mobile-390x844-1940s.png`,fullPage:true});await mobile.close();

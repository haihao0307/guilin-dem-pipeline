...
try{
  ...
  const verifyPatch=(id)=>{
    const p=manifest.patches.find(x=>x.id===id),s=history.summary.patches.find(x=>x.id===id),source=p.roads.sourceModernClassPartCounts||{},active=p.roads.classPartCounts||{},motorway=(source.motorway||0)+(source.motorway_link||0),construction=source.construction||0,bridge=s.majorBridge||0;
    check(!('motorway' in active),`${id}: motorway class survived`);check(!('motorway_link' in active),`${id}: motorway_link class survived`);check(!('construction' in active),`${id}: construction class survived`);check((s.construction||0)===construction,`${id}: construction subtraction mismatch`);check(p.roads.sourcePartCount-p.roads.partCount===motorway+construction+bridge,`${id}: source/filtered part delta mismatch`);for(const[name,count]of Object.entries(s.majorBridgeByClass||{}))check((source[name]||0)-(active[name]||0)===count,`${id}: ${name} bridge subtraction mismatch`);return{p,s,motorway,construction,bridge};
  };
  ...
  const historicalSegments=Number(await page.locator('#terrain').getAttribute('data-history-1953-control-segments')),terrainAnchored=Number(await page.locator('#terrain').getAttribute('data-history-1953-terrain-anchored')),referenceAnchored=Number(await page.locator('#terrain').getAttribute('data-history-1953-reference-anchored')),frameRejected=Number(await page.locator('#terrain').getAttribute('data-history-1953-frame-segments-rejected'));
  check(historicalSegments>0,'1953 control layer generated no georeferenced segments');check(referenceAnchored>0,'1953 sea-gap reference anchoring was not exercised');check(terrainAnchored>=0,'1953 terrain anchor metric missing');check(frameRejected>=0,'1953 frame rejection metric missing');
  await hb.click();check((await hb.getAttribute('aria-pressed'))==='true','1953 toggle did not turn on');check(await page.locator('#history-1953-layer-card').isVisible(),'1953 control card not visible after toggle on');await page.screenshot({path:`${out}/desktop-overview-1953-on.png`,fullPage:true});
  await hb.click();check((await hb.getAttribute('aria-pressed'))==='false','1953 toggle did not turn off');check(!(await page.locator('#history-1953-layer-card').isVisible()),'1953 control card still visible after toggle off');await page.screenshot({path:`${out}/desktop-overview.png`,fullPage:true});

  const mobile=await browser.newContext({viewport:{width:390,height:844},reducedMotion:'reduce'});if(target.includes('githack'))await mobile.setExtraHTTPHeaders({Cookie:'__Http-phish=1'});const mp=await mobile.newPage();await mp.goto(target,{waitUntil:'domcontentloaded',timeout:120000});await mp.waitForFunction(()=>document.documentElement.dataset.wenzhouHistoryMode==='1942-preclean-r3',{},{timeout:120000});check(await mp.locator('#history-1942-card').count()===1,'mobile history status card missing');const brand=await mp.locator('.brand p').textContent();check(brand?.includes('1942/1953'),'mobile header does not identify historical preclean');const mhb=await historyButton(mp);check((await mhb.getAttribute('aria-pressed'))==='false','mobile 1953 layer must default off');await mhb.click();check((await mhb.getAttribute('aria-pressed'))==='true','mobile 1953 toggle did not turn on');check(await mp.locator('#history-1953-layer-card').count()===1,'mobile 1953 layer card missing');await mp.screenshot({path:`${out}/mobile-390x844-1953-on.png`,fullPage:true});await mobile.close();
  console.log(JSON.stringify({passed:!failures.length,target,removedMotorwayParts:history.summary.motorway,removedMotorwayLinkParts:history.summary.motorwayLink,removedConstructionParts:history.summary.construction,removedMajorBridgeParts:history.summary.majorBridge,removedMajorBridgeByClass:history.summary.majorBridgeByClass,removedRoadParts:history.summary.removedParts,removedRoadSegments:history.summary.removedSegments,historical1953Layer:{segments:historicalSegments,terrainAnchored,referenceAnchored,frameSegmentsRejected:frameRejected,defaultVisible:false,togglePassed:true},overview:{sourceParts:overview.roads.sourcePartCount,filteredParts:overview.roads.partCount,removedMotorwayClasses:overviewCheck.motorway,removedConstructionParts:overviewCheck.construction,removedMajorBridgeParts:overviewCheck.bridge},query02:{sourceParts:q2.roads.sourcePartCount,filteredParts:q2.roads.partCount,removedParts:q2summary.removedParts,removedConstructionParts:q2summary.construction,removedMajorBridgeParts:q2summary.majorBridge},failures},null,2));

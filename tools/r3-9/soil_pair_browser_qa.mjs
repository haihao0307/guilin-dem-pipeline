...
const packedNames=()=>[...new Set(packed().map(u=>{try{return new URL(u).pathname.split('/').at(-1);}catch{return u;}}))];
...
try{
  ...
  check(packedNames().length===1,'initial soil should fetch exactly one logical packed pair');
  ...
  check(packedNames().length>=4,'pair switching did not request distinct packed payloads');
  ...
  console.log(JSON.stringify({passed:!failures.length,target,pairCount:index.pairCount,rawBytes:index.rawBytes,compressedBytes:index.compressedBytes,reduction:index.reduction,logicalPackedRequests:packedNames().length,networkPackedRequests:packed().length,legacyRequests:legacy().length,failures},null,2));

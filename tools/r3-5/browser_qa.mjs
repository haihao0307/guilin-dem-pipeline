...
assert(manifest.schema==='wenzhou-r3.5-osm-browser-bundle/r2','manifest schema is not r2');
...
assert(manifest.sourceReleaseAssetSha256==='f3ae1c9051b25fc1d23c8df1dd951a9138d6b188b1e46bff56a352c324a90101','source release SHA changed');
assert(manifest.sourceCorrectedReportSha256==='d9a2d7986f74cfd4309174151dbc36920e188080f4c1daf21ae865efa3dd4364','corrected report SHA changed');
...
assert(manifest.buildingPolicy.includes('before clipping')&&manifest.buildingPolicy.includes('synthetic closure'),'building no-closure policy missing');
...
async function state(){return page.evaluate(()=>{const c=document.querySelector('#terrain');return{
  ...
  buildingBoundaryParts:Number(c?.dataset.osmBuildingBoundaryParts),buildingSegments:Number(c?.dataset.osmBuildingSegmentsDrawn),buildingRejected:Number(c?.dataset.osmBuildingSegmentsRejectedNoSurface),buildingSyntheticPatchClosure:c?.dataset.osmBuildingSyntheticPatchClosure,
  ...
  sourceSha:c?.dataset.osmSourceReleaseSha256,correctedReportSha:c?.dataset.osmCorrectedReportSha256,quantStep:Number(c?.dataset.osmQuantizationMaxStepM),
...
assert(s.buildingBoundaryParts>0&&s.buildingSegments>0,`query-01 building boundaries missing ${JSON.stringify(s)}`);
assert(s.buildingSyntheticPatchClosure==='false',`query-01 synthetic patch closure claim changed ${JSON.stringify(s)}`);
...
assert(s.sourceSha==='f3ae1c9051b25fc1d23c8df1dd951a9138d6b188b1e46bff56a352c324a90101','runtime source release SHA changed');
assert(s.correctedReportSha==='d9a2d7986f74cfd4309174151dbc36920e188080f4c1daf21ae865efa3dd4364','runtime corrected report SHA changed');
...
s=await state();assert(s.surfaceAnchor==='current-display-triangles'&&s.buildingHeightClaim==='unknown-not-generated'&&s.buildingSyntheticPatchClosure==='false','R3.5 claims changed after 3x rebuild');
...
for(const id of ['river-oujiang','mountains','overview']){
  await selectPatch(id);s=await state();assert(s.roadParts>0&&s.roadSegments>0,`${id} roads missing ${JSON.stringify(s)}`);assert(s.seaKind==='demonstration',`${id} sea missing`);assert(s.buildingSyntheticPatchClosure==='false',`${id} synthetic closure boundary changed`);
  if(id==='overview'){assert(s.buildingBoundaryParts===0&&s.buildingSegments===0&&s.buildingsVisible==='false',`overview must omit buildings ${JSON.stringify(s)}`);assert(s.quantStep>3&&s.quantStep<4,`overview quantization policy ${JSON.stringify(s)}`);}else{assert(s.quantStep<1,`${id} quantization should remain sub-meter ${JSON.stringify(s)}`);}
...
assert(mobileLayout.osmKind==='external-mapped-observation'&&mobileLayout.roadSegments>0&&mobileLayout.buildingBoundaryParts>0&&mobileLayout.buildingSegments>0,'mobile OSM evidence missing');
assert(mobileLayout.buildingSyntheticPatchClosure==='false','mobile synthetic closure claim changed');

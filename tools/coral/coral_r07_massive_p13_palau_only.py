...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: coral_r07_massive_p13_palau_only.py <build-dir>")
    ...
    html = html.replace("NOAA Massive Coral P12", "NOAA Massive Coral P13 · Palau Only")
    html = html.replace("Massive Coral P12", "Massive Coral P13 · Palau Only")
    html = html.replace("R07-P12", "R07-P13")
    html = html.replace("version:'R07-P12'", "version:'R07-P13'")
    html = html.replace(
    ...
    html = html.replace("assetStatus:'morphology prototype'", "assetStatus:'palau-verified-species-morphology-candidate'")

    old_gate = "localSitePlacementEvidence:'UNRESOLVED_AIRAI_STONE_MONEY_ISLAND',ecologicalPlacementReady:false"
    new_gate = (
        ...
        "localSitePlacementEvidence:'UNRESOLVED_AIRAI_STONE_MONEY_ISLAND',ecologicalPlacementReady:false"
    )
    gate_count = html.count(old_gate)
    require(gate_count == 2, f"expected two runtime Palau gate insertion points, found {gate_count}")
...
'''.strip()
    ...
    require("palauOnlyProductionEligible:true" in html, "P13 eligibility runtime marker missing")
    require("productionAdmission:'PALAU_ONLY_ADMITTED'" in html, "P13 admission marker missing")
    ...
    classification_path.write_text(
        json.dumps(classification_doc, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
    ...
    (root / "P13_PALAU_ONLY_ZH.md").write_text(
        ...
        "- P13 保留 P12 测地半球与 Poisson 珊瑚杯几何，不用重新命名物种来匹配现有模型。\n"

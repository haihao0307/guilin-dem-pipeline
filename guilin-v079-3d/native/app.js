...
void main() {
  ...
  async function loadTile(tileId, anchorId = null) {
    ...
    if (!cache) {
      const buffer = await fetchBinary(tile.file.startsWith('http') ? tile.file : `data/${tile.file}`);
  ...
  function updateQaResult() {
    ...
    const result = {
      ...
      public_deployment_allowed: true,
  ...
  async function initialize() {
    ...
    requestAnimationFrame(renderLoop);
    window.__GUILIN_V079_TEST_API = window.__GUILIN_V077_TEST_API = {

...
const manifest = await fetch('manifest.json?v=5', { cache: 'no-store' }).then(async response => {
...
async function start3D(gl) {
  ...
  const [heightImage, surfaceImage, waterMaskImage, flowImage, waterLevelImage] = await Promise.all([
    ...
    loadImage('assets/osm_water_mask_v004.png'),
...
void main(){
  ...
  if(uMode==0){
    vec3 yunnan=saturation(surface,.94+.28*uRichness);
    float moistMask=clamp((1.0-t)*flatTerrain*(.35+.65*uMoisture),0.0,1.0);
    yunnan=mix(yunnan,vec3(.12,.31,.16),moistMask*.14*uMoisture);
    float redSoil=clamp(slope*(.28+.72*uRock)+(t-.42)*.24,0.0,1.0);
    yunnan=mix(yunnan,vec3(.48,.27,.18),redSoil*.14*uRock);
    float rockMask=clamp(slope*.72+(t-.66)*1.1,0.0,1.0);
    yunnan=mix(yunnan,vec3(.48,.45,.42),rockMask*.20*uRock);
    color=yunnan*illumination;
  }else if(uMode==1){
  ...
  vec3 flowTex=texture(uFlow,vUV).rgb;
  ...
  vec2 direction=flowTex.rg*2.0-1.0;
  float directionLength=length(direction);
  direction=directionLength>.08?normalize(direction):vec2(0.0,1.0);
  float flowConfidence=smoothstep(.10,.65,flowTex.b);
  float flowPhase=dot(vUV*vec2(2048.0,2814.0),direction)*.10-uTime*(.55+uFlowSpeed*3.2)*mix(.35,1.0,flowConfidence);
  float flowBands=clamp((.5+.5*sin(flowPhase)+.22*sin(flowPhase*2.17+1.3))*.64,0.0,1.0);
  float waveA=sin(vWorld.x*.015+vWorld.z*.010+uTime*(.42+uWave*1.8));
  float waveB=sin(vWorld.x*.024-vWorld.z*.017-uTime*(.31+uWave*1.25));
  float lakeWaves=.5+.25*waveA+.25*waveB;
  float animation=mix(flowBands,lakeWaves,lake);
  vec3 deep=mix(vec3(.025,.18,.34),vec3(.035,.34,.56),uWaterColor);
  vec3 bright=mix(vec3(.10,.47,.66),vec3(.23,.72,.91),uWaterColor);
  vec3 waterColor=mix(deep,bright,.16+.32*animation*(.35+.65*uWave));
  float opacity=mix(.46,.78,uWaterColor);
  color=mix(color,waterColor,water*opacity);
  float glint=pow(max(0.0,.5+.5*sin(vWorld.x*.032+vWorld.z*.028+uTime*1.05)),18.0);
  color+=water*vec3(.36,.49,.55)*glint*uWave*.30;
  ...
  bindRange('flowSpeed', 'flowSpeed', 'flowSpeedOut');
  bindRange('wave', 'wave', 'waveOut');
  ...
  function render() {
    ...
    statusEl.textContent = `V005 · 北向已校正 · ${desktopHigh ? '4096 级色彩' : '2048 级色彩'} · ${manifest.hydrology.clippedWaterwayFeatures.toLocaleString('zh-CN')} 条 OSM 水路线 · ${manifest.hydrology.clippedWaterAreaFeatures.toLocaleString('zh-CN')} 个水体面 · 镜头离地约 ${clearance.toFixed(clearance < 100 ? 1 : 0)} m`;

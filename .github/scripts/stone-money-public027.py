"""Verify exact immutable public Stone Money Island V0.2.7 bytes and browser entry."""
...
OUT=Path('games/survivor-palau/releases/v0.2.7');SHOTS=Path('/tmp/smi-public027');SHOTS.mkdir(exist_ok=True)
proof={'version':'0.2.7','url':URL,'entrySha256':EXPECTED,'sourceCommit':os.environ['SMI_SOURCE_COMMIT'],'pagesCommit':os.environ['SMI_PAGES_COMMIT'],'publicHttpsPassed':False,'browserPassed':False,'shareAllowed':False,'visualAcceptance':False,'physicalDeviceTest':False,'cases':[]}
def save():
 ...
 with sync_playwright() as p:
  ...
  for name,w,h in [('desktop',960,540),('mobile',390,844)]:
   ...
   try:
    ...
    diag=page.evaluate('StoneMoneySurvival.diagnostics()');assert diag['archGeometry']=='dissolved-mushroom-rock-island-v027';assert diag['archOpening']['secondarySeaCave'] is True and diag['archOpening']['offCenter'] is True;assert diag['archMass']['vegetationPockets']==7 and abs(diag['archMass']['tidalToCrownWidthRatio']-.46)<.001 and diag['archMass']['minimumRoofThickness']>=5.7;case['arch']={'opening':diag['archOpening'],'mass':diag['archMass']}

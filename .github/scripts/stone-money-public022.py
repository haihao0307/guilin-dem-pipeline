...
try:
 ...
 with sync_playwright() as p:
  browser=p.chromium.launch(executable_path=shutil.which('google-chrome'),headless=True,args=['--no-sandbox','--use-gl=angle','--use-angle=swiftshader','--enable-unsafe-swiftshader','--disable-dev-shm-usage'])
  for name,w,h in [('desktop',960,540),('mobile',390,844)]:
   ...
   try:
    ...
    error=page.locator('#error').inner_text();assert not error,error;assert page.evaluate('OceanIsland.qa.ready && StoneMoneySurvival?.version === "0.2.2"')
 ...
 proof['chapterLoopPassed']=False # Filled only by the aggregate job using the actual fish regression receipt.
...
sys.exit(0 if proof['browserPassed'] and proof['publicHttpsPassed'] else 1)

"""Verify the exact immutable public V0.2.4 workbench on desktop and phone-size viewports."""
...
OUT=Path('games/survivor-palau/releases/v0.2.4');SHOTS=Path('/tmp/smi-public024');SHOTS.mkdir(exist_ok=True)
proof={'version':'0.2.4','url':URL,'entrySha256':EXPECTED,'sourceCommit':os.environ['SMI_SOURCE_COMMIT'],'pagesCommit':os.environ['SMI_PAGES_COMMIT'],'publicHttpsPassed':False,'browserPassed':False,'shareAllowed':False,'visualAcceptance':False,'physicalDeviceTest':False,'cases':[]}
def save():
...
try:
 for attempt in range(24):
  try:
   ...
   proof['httpStatus']=status;proof['actualSha256']=hashlib.sha256(data).hexdigest()
   assert status==200 and proof['actualSha256']==EXPECTED

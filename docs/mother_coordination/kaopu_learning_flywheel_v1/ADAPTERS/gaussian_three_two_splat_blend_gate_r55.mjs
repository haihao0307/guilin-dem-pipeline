export function validateTwoSplatBlendR55(report){
  ...
  for(const side of['webgl','webgpu']){
    const r=report?.[side]||{},s=r.summary||{},m=s.floatVsByte||{},e=s.floatVsExpected||{},o=s.order||{};
    ...
    if(!(e.maxAbs<=1e-5))errors.push(`${side}-float-reference-mismatch`);
    if(!(o.maxRgbAbs>1e-4&&o.changedRgbChannels>0))errors.push(`${side}-reverse-control-did-not-change-rgb`);
    if(!(o.maxAlphaAbs<=1e-6))errors.push(`${side}-reverse-control-changed-alpha`);

...
const webgl=runs.filter(r=>r.status==='candidate-observation'&&r.actualBackend==='webgl');
const half=webgl.filter(r=>r.bufferName==='default'),flt=webgl.filter(r=>r.bufferName==='float');
const webgpuUnavailable=runs.filter(r=>r.requestedMode==='webgpu'&&r.status==='error'&&String(r.message).includes('valid external Instance reference no longer exists'));
const comparison={schema:'kaopu-three-output-buffer-comparison/r58',status:'Candidate-observation',fixture:{sampleCount:N,source:'exact R55 stepwise float32 premultiplied output',sourceHash:'0ca00d66d1646fd820cf6fca230460f4dd11b31d05c1a683aa12278603a1e520',outputColorSpace:'SRGBColorSpace forces the r186 internal framebuffer path',toneMapping:'NoToneMapping',bufferCases:['constructor default omitted (HalfFloatType)','explicit FloatType'],presentation:'opaque-black Playwright PNG; RGB checked separately from internal target'},
  ...
  checks:{twoWebglSuccessfulPages:webgl.length===2,allWebglPageChecks:webgl.length===2&&webgl.every(r=>Object.values(r.checks).every(Boolean)),sourceLocked:webgl.length===2&&webgl.every(r=>r.source.hash===r.source.lockedHash),defaultIsHalfFloat:half.length===1&&half[0].internalTarget.rendererOutputBufferType===half[0].internalTarget.expectedType&&half[0].internalTarget.rawConstructor==='Uint16Array',explicitIsFloat:flt.length===1&&flt[0].internalTarget.rendererOutputBufferType===flt[0].internalTarget.expectedType&&flt[0].internalTarget.rawConstructor==='Float32Array',floatPathMatchesSource:flt.length===1&&flt[0].internalTarget.metricsAgainstSource.maxAbs<=1e-6,halfPathFiniteAndBounded:half.length===1&&half[0].internalTarget.finiteChannels===1024&&half[0].internalTarget.metricsAgainstSource.maxAbs>0&&half[0].internalTarget.metricsAgainstSource.maxAbs<=0.0005,webglCanvasClassified:Boolean(comparisons.webgl)&&comparisons.webgl.halfCanvas.maxCodeDiff<=2&&comparisons.webgl.floatCanvas.maxCodeDiff<=2&&comparisons.webgl.canvasHalfVsFloatRgb.maxCodeDiff<=2,webgpuReadbackUnavailableClassified:webgpuUnavailable.length===2},
  interpretation:{candidate:'For the bounded [0,1] R55 fixture, the r186 default path must be judged from observed RGBA16F storage error separately from the later sRGB/canvas quantization.',counterexample:'A matching or smaller final 8-bit difference cannot prove the internal Float and HalfFloat paths are equivalent.',webgpuBoundary:'Both internal-target readbacks reached the same external-instance mapAsync failure as R57; this run therefore adds no WebGPU numeric comparison.',sameChromiumSwiftShaderEvidenceRoot:true},limits:{webgpuInternalReadback:'Unknown in this execution environment',fixtureErrorBoundsAreNotAssetAcceptanceThresholds:true,privateRevisionPinnedProbe:true,hardwareGpu:false,targetDevice:false,appleSafariWebKit:false,realPhotoOrLearnedAsset:false,humanAcceptance:false}};

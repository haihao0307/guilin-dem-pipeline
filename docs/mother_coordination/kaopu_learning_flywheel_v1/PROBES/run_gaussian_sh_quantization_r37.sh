...
out_dir="${1:-$probe_dir/.gaussian-sh-r37-out}"
mkdir -p "$out_dir"

g++ -std=c++17 -O2 \
  -I "$probe_dir/gaussian_spz_r34_support" \
  -I "$SPZ_ROOT/src/cc" \
  "$probe_dir/gaussian_sh_quantization_r37.cc" \
  "$SPZ_ROOT/src/cc/load-spz.cc" \
  "$SPZ_ROOT/src/cc/splat-c-types.cc" \
  "$SPZ_ROOT/src/cc/splat-types.cc" \
  -lz -Wl,-l:libzstd.so.1 \
  -o "$out_dir/gaussian_sh_quantization_r37"

"$out_dir/gaussian_sh_quantization_r37" "$out_dir/in-range.spz" "$out_dir/out-of-range.spz" \
  | tee "$out_dir/generator-full.log" | tail -1 > "$out_dir/metadata.json"

THREE_R186_ROOT="$THREE_R186_ROOT" node \
  "$probe_dir/gaussian_sh_quantization_r37.mjs" \
  "$out_dir/in-range.spz" "$out_dir/out-of-range.spz" "$out_dir/metadata.json" \

...
async function run(output){
  ...
  const png=PNG.sync.read(screenshotBuffer),screenshot=Buffer.from(png.data),swapchain=result.swapchain.rgba?Buffer.from(result.swapchain.rgba):null,expected=expectedBytes(r55Premultiplied(),output);
  delete result.swapchain.rgba;
  const record={...result,expected:{sha256:sha(expected)},swapchain:{...result.swapchain,metrics:swapchain?byteMetrics(swapchain,expected):null,blankTransparentPattern:swapchain?blankTransparent(swapchain):null},screenshot:{location:'Playwright transparent PNG after the immediate copy attempt completed',sha256:sha(screenshot),metricsAgainstExpected:byteMetrics(screenshot,expected),metricsAgainstSwapchain:swapchain?byteMetrics(screenshot,swapchain):null,blankTransparentPattern:blankTransparent(screenshot),rgba:Array.from(screenshot)}};
...
const directSuccess=successful.length===2&&successful.every(r=>r.swapchain.status==='candidate-observation');
const directUnavailable=successful.length===2&&successful.every(r=>r.swapchain.status==='candidate-unavailable'&&r.swapchain.error?.stage==='copy-submitted-map-pending'&&r.swapchain.error?.message.includes('valid external Instance reference no longer exists'));
const comparison={schema:'kaopu-three-webgpu-swapchain-comparison/r57',status:'Candidate-observation',fixture:{sampleCount:N,source:'exact R55 stepwise float32 premultiplied output',sourceHash:'0ca00d66d1646fd820cf6fca230460f4dd11b31d05c1a683aa12278603a1e520',rendererOutputColorSpaces:['LinearSRGBColorSpace','SRGBColorSpace'],toneMapping:'NoToneMapping',readback:'same-task GPUCanvasContext current texture copied to MAP_READ buffer after synchronous Three render',screenshotControl:'Playwright transparent PNG after immediate copy attempt'},
  observations:Object.fromEntries(runs.map(r=>[r.outputName||'error',{status:r.status,format:r.contract?.format||null,positiveControl:r.positiveControl||null,swapchainStatus:r.swapchain?.status||null,swapchainHash:r.swapchain?.rgbaHash||null,swapchainMetrics:r.swapchain?.metrics||null,swapchainBlank:r.swapchain?.blankTransparentPattern??null,screenshotHash:r.screenshot?.sha256||null,screenshotMetricsAgainstExpected:r.screenshot?.metricsAgainstExpected||null,screenshotMetricsAgainstSwapchain:r.screenshot?.metricsAgainstSwapchain||null,screenshotBlank:r.screenshot?.blankTransparentPattern??null,error:r.swapchain?.error||(r.status==='error'?{message:r.message,progress:r.progress}:null)}])),
  checks:{twoSuccessfulPages:successful.length===2,allPageChecks:successful.length===2&&successful.every(r=>Object.values(r.checks).every(Boolean)),sourceLocked:successful.length===2&&successful.every(r=>r.source.hash===r.source.lockedHash),offscreenPositiveControlPass:successful.length===2&&successful.every(r=>r.positiveControl.metrics.maxAbs<=1e-6),swapchainOutcomeClassified:(directSuccess&&successful.every(r=>r.swapchain.blankTransparentPattern===false&&r.swapchain.metrics.maxCodeDiff<=2)&&successful[0].swapchain.rgbaHash!==successful[1].swapchain.rgbaHash)||directUnavailable,screenshotControlClassified:successful.length===2&&successful.every(r=>typeof r.screenshot.blankTransparentPattern==='boolean')},
  directOutcome:directSuccess?'candidate-observation':directUnavailable?'candidate-unavailable-stable':'unclassified',

...
out_dir="${1:-$probe_dir/.gaussian-antialias-r38-out}"
mkdir -p "$out_dir"

g++ -std=c++17 -O2 \
  -I "$probe_dir/gaussian_spz_r34_support" \
  -I "$SPZ_ROOT/src/cc" \
  "$probe_dir/gaussian_antialias_handoff_r38.cc" \
  "$SPZ_ROOT/src/cc/load-spz.cc" \
  "$SPZ_ROOT/src/cc/splat-c-types.cc" \
  "$SPZ_ROOT/src/cc/splat-types.cc" \
  -lz -Wl,-l:libzstd.so.1 \
  -o "$out_dir/gaussian_antialias_handoff_r38"

"$out_dir/gaussian_antialias_handoff_r38" \
  "$out_dir/brush-mip.ply" "$out_dir/default-cli.spz" "$out_dir/explicit-mip.spz" \
  | tee "$out_dir/niantic-full.log" | tail -1 > "$out_dir/niantic.json"

THREE_R186_ROOT="$THREE_R186_ROOT" node \
  "$probe_dir/gaussian_antialias_handoff_r38.mjs" \
  "$out_dir/brush-mip.ply" "$out_dir/default-cli.spz" "$out_dir/explicit-mip.spz" "$out_dir/niantic.json" \

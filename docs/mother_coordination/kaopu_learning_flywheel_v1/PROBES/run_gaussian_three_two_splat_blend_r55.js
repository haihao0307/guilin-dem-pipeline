...
const comparison={schema:'kaopu-three-two-splat-blend-comparison/r55',status:'candidate-observation',
  ...
  crossBackend:{inputHashEqual:webgl.summary?.hashes?.inputs===webgpu.summary?.hashes?.inputs,floatHashEqual:webgl.summary?.hashes?.float===webgpu.summary?.hashes?.float,byteHashEqual:webgl.summary?.hashes?.byte===webgpu.summary?.hashes?.byte,reverseHashEqual:webgl.summary?.hashes?.reverseFloat===webgpu.summary?.hashes?.reverseFloat,maxBlendErrorDelta:(webgl.summary&&webgpu.summary)?Math.abs(webgl.summary.floatVsByte.maxAbs-webgpu.summary.floatVsByte.maxAbs):null},
  checks:{webglPass:webgl.status==='candidate-observation'&&webgl.actualBackend==='webgl'&&Object.values(webgl.checks||{}).every(Boolean),webgpuPass:webgpu.status==='candidate-observation'&&webgpu.actualBackend==='webgpu'&&Object.values(webgpu.checks||{}).every(Boolean),sameThreeRevision:String(webgl.threeRevision)==='186'&&String(webgpu.threeRevision)==='186',sameFixture:webgl.fixture?.sampleCount===256&&webgpu.fixture?.sampleCount===256,inputIdentityEqual:webgl.summary?.hashes?.inputs===webgpu.summary?.hashes?.inputs},

...
async function run(mode,activateFloatBlend=true,label=mode){
  ...
  const url=`http://127.0.0.1:8765/docs/mother_coordination/kaopu_learning_flywheel_v1/PROBES/gaussian_three_two_splat_blend_r55.html?mode=${mode}&activateFloatBlend=${activateFloatBlend?'1':'0'}`;let result;
  ...
  await page.close();fs.writeFileSync(`r55-result-${label}.json`,JSON.stringify(result,null,2)+'\n');return result;
}
const webgl=await run('webgl',true,'webgl'),webgpu=await run('webgpu',true,'webgpu'),webglListedOnly=await run('webgl',false,'webgl-listed-only');await browser.close();
const comparison={schema:'kaopu-three-two-splat-blend-comparison/r55',status:'candidate-observation',
  ...
  activationControl:{requestedMode:'webgl',floatBlendListed:true,floatBlendActivated:false,status:webglListedOnly.status,message:webglListedOnly.message||null,interpretation:'Fresh WebGL context lists EXT_float_blend but deliberately does not call getExtension before the same RGBA32F blend attempt.'},
  ...
  checks:{webglPass:webgl.status==='candidate-observation'&&webgl.actualBackend==='webgl'&&webgl.fixture?.floatBlendCapability?.floatBlendActivated===true&&Object.values(webgl.checks||{}).every(Boolean),webgpuPass:webgpu.status==='candidate-observation'&&webgpu.actualBackend==='webgpu'&&webgpu.fixture?.floatBlendSupported===true&&Object.values(webgpu.checks||{}).every(Boolean),listedOnlyNegativeControlFails:webglListedOnly.status==='error',sameThreeRevision:String(webgl.threeRevision)==='186'&&String(webgpu.threeRevision)==='186',sameFixture:webgl.fixture?.sampleCount===256&&webgpu.fixture?.sampleCount===256,inputIdentityEqual:webgl.summary?.hashes?.inputs===webgpu.summary?.hashes?.inputs},

...
export class Renderer{
 ...
 async load(data){
  ...
  this.state='ready';this.bookmark('overview');this.frameTimes=[];this.frames=0;
 }
 ...
 render(){const gl=this.gl,c=this.canvas,w=Math.max(1,c.clientWidth),h=Math.max(1,c.clientHeight);if(c.width!==w||c.height!==h){c.width=w;c.height=h}gl.viewport(0,0,w,h);gl.clearColor(.72,.77,.79,1);gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);gl.disable(gl.DEPTH_TEST);gl.disable(gl.CULL_FACE);gl.useProgram(this.skyProgram);gl.bindVertexArray(null);gl.drawArrays(gl.TRIANGLES,0,3);if(this.state!=='ready')return;
  ...
  gl.bindVertexArray(null);this.drawCalls=calls;this.triangles=tri;this.frames++;
 }
 loop(t){const elapsed=this.lastTime?Math.min((t-this.lastTime)/1000,.1):0;this.lastTime=t;
  ...
  const sig=JSON.stringify([this.state,this.target,this.yaw,this.pitch,this.distance,this.settings,this.canvas.clientWidth,this.canvas.clientHeight]);const changed=sig!==this.lastSignature;this.lastSignature=sig;const start=performance.now();try{if(changed)this.render()}catch(e){this.state='error';this.error=e.message;console.error(e)}const done=performance.now();if(changed&&this.state==='ready'){this.frameTimes.push({time:t,cpuMs:done-start});if(this.frameTimes.length>180)this.frameTimes.shift()}requestAnimationFrame(this.loop)
 }
 snapshot(){const f=this.frameTimes,seconds=f.length>1?(f.at(-1).time-f[0].time)/1000:0;return {state:this.state,caseId:this.recipe?.id,grid:2049,spacingM:1,geometryVertices:this.audit?.gridVertices,terrainTriangles:this.audit?.terrainTriangles,geometryCameraDependent:false,geometryDeviceDependent:false,textureCount:0,lodCount:0,drawCalls:this.drawCalls,visibleTriangles:this.triangles,frameCount:this.frames,fps:seconds?(f.length-1)/seconds:null,eye:this.eye(),target:[...this.target],error:this.error||null}}
}

...
async function capture(name, viewport, isMobile) {
  ...
  await page.goto(baseUrl, { waitUntil: 'networkidle', timeout: 240_000 });
  ...
  if (qa.truth_grid?.[0] !== 81 || qa.render_grid?.[0] !== 641) throw new Error(`${name}: truth/render grid mismatch`);
  if (qa.render_spacing_m !== 1.5625 || qa.render_subdivision_factor !== 8) throw new Error(`${name}: render subdivision mismatch`);
  ...
  if (!(qa.terrain_vertex_count >= 400_000 && qa.terrain_triangle_count >= 800_000)) throw new Error(`${name}: 3D geometry density too low`);
  ...
  const set = async (mode, view, wait = 420) => {
...
const report = {
  schema: 'guilin-dem-mother-sample-v002-browser-evidence/v3',
